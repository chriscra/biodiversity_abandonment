...
---
```{r initialize}
source("/Users/christophercrawford/Google_Drive/_Projects/abandonment_biodiversity/scripts/0_start.R")
```
...
# what projections should I use to reproject the various rasters?
bt
plot(bt$y1987)
bt_reproj <- projectRaster(bt, crs = crs("+proj=bonne +lat_1=10 +lon_0=30"), method = "ngb")

plot(bt_reproj$y1987)


# load in 11 sites, try reprojecting with the same CRS, and test the results
...
# make a list with 11 reprojections
brtest <- brick(paste0(p_dat_derived, "belarus_reproj.tif"))
srtest <- brick(paste0(p_dat_derived, "shaanxi_reproj.tif"))
crs(brtest)
crs(srtest)

# belarus & shaanxi
CRS("+proj=bonne +lat_1=10 +lon_0=30 +x_0=0 +y_0=0 +ellps=WGS84 +units=m +no_defs")



# write a function to load, reproject, and write out new raster
...
write_csv(lsm_test, path = paste0(p_dat_derived, frag, "/", run_label, ".csv"))
...
```{r note!}
# *check notes on David's thoughts on metrics, in Evernote, "David \\ 2020.10.24"

# The SHI (Species Habitat Index) includes a measure of connectivity: "the average distance to edge of suitable area".
# See https://mol.org/indicators/habitat/background
...
cc_clean_frag_results(site = "shaanxi", n_metrics = 15)
load(file = paste0(p_output, "frag/frag_", "shaanxi", ".rds"),
         verbose = TRUE)

cc_clean_frag_results(site = "belarus", n_metrics = 15)
