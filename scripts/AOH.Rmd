...
aoh_types <- aoh_type_df$label[c(4, 6, 8)]
aoh_type <- aoh_types[3]
...
aoh_l <- lapply(aoh_types, 
  ...
  lapply(1:21, 
    ...
    read_csv(paste0(p_derived, "aoh/aoh_tmp_", aoh_type,
                    ifelse(grepl("max", aoh_type), 
                           ifelse(grepl("potential", aoh_type),
                                  aoh_run_date, "_2022_04_08"), "_2022_04_01"),
                    "_c", i, ".csv"))
    }
    ) %>% 
    bind_rows() %>% 
...
aoh_l %>% group_by(aoh_type, site) %>% summarise(n_sp = length(unique(binomial))) %>%
...
seff %>%
  filter(label == "aoh_hab8") %>% arrange(array) %>% print(n = 30)
