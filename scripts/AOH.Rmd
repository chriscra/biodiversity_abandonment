...
gg_aoh_sp_obs_by_site$crop_abn_iucn
...
{
tic()
lapply(aoh_types_sub, #aoh_types, 
       function(aoh_type_) {
         aoh_tmp <- aoh %>% 
...
aoh_feols_trends <- 
  ...
  mutate(
    aoh_type = fct_relevel(aoh_type, aoh_types_sub),#[c(6, 7, 3, 5, 1, 2, 4)]),
...
```{r plot-feols}
lapply(
  aoh_types_sub,# aoh_types[c(6,3,1,7,5,8)],
  # "crop_abn_iucn",
  function(i) {
  gg_aoh_feols_overall_tmp <-
    aoh_feols_trends_by_sp %>%
    filter(passage_type == "exclude_passage",
         vert_class != "amp",
         !(vert_class == "bird" & mature_forest_obl > 0.5),
  ...
  ggsave(plot_grid(gg_aoh_feols_overall_tmp,
                   gg_aoh_feols_by_site_tmp,
                   nrow = 1, ncol = 2, 
                   # labels = "auto",
                   align = "h", axis= "b",
         ...
         filename = paste0(p_output, "plots/aoh/", 
                           "aoh_feols_overall_w_site_combo_",
                           i, aoh_run_date, 
                           # "_no_lab",
                           ".pdf"),
...
```{r compare-mean-AOH-start-to-end}
tmp_df
...
aoh_start_end_l <- 
  lapply(c(1, seq(from = 5, to = 15, by = 5)), 
         function(i) {
           left_join(
             aoh %>% filter(aoh_type == "crop_abn_iucn") %>% group_by(run_index) %>% slice_min(year, n = i) %>% 
               summarise(start = mean(aoh), start_year = min(year)) %>% ungroup(),
             aoh %>% filter(aoh_type == "crop_abn_iucn") %>% group_by(run_index) %>% slice_max(year, n = i) %>% 
               summarise(end = mean(aoh), end_year = max(year)) %>% ungroup()) %>%
             mutate(window_size = i)
}) %>% 
  bind_rows() %>%
  left_join(run_indices, by = "run_index") %>%
  left_join(select(area_summary_df, site, total_site_area_ha_2017), by = "site") %>%
  ...
  mutate(abs_change = end - start, 
         ...
         abs_change_as_percent_site_area = abs_change / total_site_area_ha_2017) %>%
  select(c("run_index", "start", "start_year", "end", "end_year", "window_size", "abs_change", "prop_change", "percent_change", "ratio", "ratio_mod", "abs_change_as_percent_site_area"), everything())
...
gg_total_p_disagree <- df_agree %>%
  ...
  facet_grid(~ window_size, labeller = label_both)
...
aoh_change_df <-
  ...
  left_join(run_indices, by = "run_index") %>%
  left_join(start_end_years_df %>% select(run_index, contains("year")), by = "run_index") %>%
  select(run_index, est_type, vert_class, site, contains("year"), slope, intercept, aoh_type, passage_type, vert_class, binomial, mature_forest_obl) %>%
  left_join(select(area_summary_df, site, total_site_area_ha_2017), by = "site") %>% 
...
aoh_change_df %>%
  filter(est_type == "estimate", aoh_type == "crop_abn_iucn", str_detect(passage_type, "excl"),
         vert_class != "amp", !(vert_class == "bird" & mature_forest_obl > 0.5))
...
aoh_change_df_tmp <- aoh_change_df %>% 
  filter(est_type == "estimate", aoh_type == "crop_abn_iucn", str_detect(passage_type, "excl"),
         vert_class != "amp", !(vert_class == "bird" & mature_forest_obl > 0.5),
         # overall_trend == "gain", rel_change < 0
         ) 

names(aoh_change_df_tmp)

aoh_start_end_l_tmp <- aoh_start_end_l %>%
      filter(aoh_type == "crop_abn_iucn", str_detect(passage_type, "excl"),
             vert_class != "amp", !(vert_class == "bird" & mature_forest_obl > 0.5))

aoh_start_end_l_tmp %>% filter(window_size == 5)

# --------------------------------------------------------------- #
# ------------ absolute AOH change, by site --------------------- #
# --------------------------------------------------------------- #
...
gg_abs_change_by_site <-
  ...
  geom_vline(data = aoh_change_df_tmp %>%
               filter(
                 # site != "mato_grosso", 
                      trend != "no trend") %>%
               group_by(trend, site) %>%
...
ggsave(
  gg_abs_change_by_site, 
  filename = paste0(p_output, "plots/aoh/effect_size/", "hist_aoh_abs_change_by_site_annotated.pdf"),
...
ggsave(gg_abs_change_by_trend,
    filename = paste0(p_output, "plots/aoh/effect_size/", "hist_aoh_abs_change_by_trend_free_annotated.pdf"),
    width = 7, height = 5, units = "in")
...
gg_abs_change_as_percent_site_area_by_trend <-
  aoh_change_df_tmp %>%
      ggplot() +
      geom_vline(xintercept = 0, linetype = "dashed", col = "gray50") +
      geom_histogram(aes(x = abs(abs_change_as_percent_site_area) * 100, fill = trend, group = trend), bins = 50, col = "black", size = 0.5) +
      facet_grid(rows = vars(trend), cols = vars(vert_class),
                 scales = "free",
                 labeller = as_labeller(aoh_type_labels)) +
      theme_classic() +
...
ggsave(gg_abs_change_as_percent_site_area_by_trend,
    filename = paste0(p_output, "plots/aoh/effect_size/", "hist_abs_change_as_percent_site_area_by_trend.pdf"),
...
ggsave(
  gg_abs_change_per_year, 
  filename = paste0(p_output, "plots/aoh/effect_size/", "hist_aoh_abs_change_per_year.pdf"),
  width = 7, height = 8.5, units = "in")
...
gg_abs_change_as_percent_site_area <- 
  aoh_change_df_tmp %>%
  ggplot() + 
  geom_histogram(aes(x = abs_change_as_percent_site_area * 100, fill = trend, group = trend), bins = 50, col = "black") +
  theme_classic() +
  scale_fill_manual(name = "Trend", labels = palette_labels[c(1, 3, 6)], values = palette_du_jour[c(1, 3, 6)]) + 
  labs(x = "Change in AOH as a percent of site area (%)", y = "Count") + 
  theme(legend.position = "bottom", strip.text.y.right = element_text(size = 8, angle = 0, hjust = 0, vjust = 0)) +
...
ggsave(
  gg_abs_change_as_percent_site_area, 
  filename = paste0(p_output, "plots/aoh/effect_size/", "hist_abs_change_as_percent_site_area.pdf"),
  width = 7, height = 8.5, units = "in")
...
ggsave(
  gg_prop_change, 
  filename = paste0(p_output, "plots/aoh/effect_size/", "hist_prop_change.pdf"),
  width = 7, height = 8.5, units = "in")
...
ggsave(
  gg_prop_change_constrained, 
  filename = paste0(p_output, "plots/aoh/effect_size/", "hist_prop_change_constrained.pdf"),
  width = 7, height = 8.5, units = "in")
...
aoh_change_df_tmp %>% 
  select(run_index, vert_class, site, trend, contains("change"),
         contains("aoh")) %>% 
  filter(abs(prop_change) < 10,
         site == "shaanxi", trend == "gain", prop_change < 0
  )
...
gg_prop_change_obs <-
  aoh_start_end_l_tmp %>% 
      filter(window_size == 1) %>%
  filter(abs(prop_change) < 30) %>%
  # filter(prop_change < 0) %>% 
  ggplot() + 
...
ggsave(
  gg_prop_change_obs, 
  filename = paste0(p_output, "plots/aoh/effect_size/", "hist_prop_change_obs.pdf"),
  width = 7, height = 8.5, units = "in")
...
lapply(c(1, 5, 10, 15), function(i) {
  ...
  theme_classic() +
  scale_fill_manual(name = "Trend", labels = palette_labels[c(1, 3, 6)], values = palette_du_jour[c(1, 3, 6)]) + 
  labs(x = "Proportional change in observed AOH", y = "Count", caption = paste0("Comparing mean values in first and last ", i, " years")) + 
  theme(legend.position = "bottom", strip.text.y.right = element_text(size = 8, angle = 0, hjust = 0, vjust = 0)) +
  facet_grid(site ~ ., scales = "free", labeller = as_labeller(fancy_labels)),
  
  filename = paste0(p_output, "plots/aoh/effect_size/", "hist_prop_change_obs_", i, ".pdf"),
  width = 7, height = 8.5, units = "in")
})
...
aoh %>%
  filter(run_index %in% weird_runs) %>%
...
lapply(c(1, 5, 10, 15), function(i) {
  ggsave(
    aoh_start_end_l_tmp %>% 
      ...
      geom_vline(data = aoh_start_end_l_tmp %>% 
    ...
    filename = paste0(p_output, "plots/aoh/effect_size/", "hist_factor_change_obs_annotated", i, ".pdf"),
    width = 7, height = 8.5, units = "in")
})
...
ggsave(
  aoh_start_end_l_tmp %>% 
      ...
      theme(legend.position = "bottom", ggh4x.axis.ticks.length.minor = rel(1)),
    filename = paste0(p_output, "plots/aoh/effect_size/", "hist_factor_change_obs_by_trend1.pdf"),
    width = 7, height = 6, units = "in")
...
ggsave(
  aoh_start_end_l_tmp %>% 
      ...
      geom_histogram(aes(x = abs(ratio_mod), fill = trend, col = trend, group = trend), bins = 50, binwidth = 0.5,
                     col = "black", size = 0.5) +
    geom_vline(data = aoh_start_end_l_tmp %>% filter(window_size == 5, 
                                                     # abs(ratio_mod) < 15,
                                                     !is.na(trend)) %>%
    ...
    filename = paste0(p_output, "plots/aoh/effect_size/", "hist_factor_change_obs_by_trend2_free_annotated.pdf"),
    width = 7, height = 5, units = "in")
...
## Developing Temporal Autocorrelation Approach
...
# temp dataset
aoh %>% filter(run_index == 4233)
aoh_trends %>% filter(run_index == 4233)
aoh_lag_trends %>% filter(run_index == 4233)
aoh_lm_lag %>% filter(run_index == 4233)

tst <- aoh %>% 
  filter(
    # run_index == 4233, 
    run_index < 4239, run_index > 4230
         ) %>% 
  select(run_index, binomial, aoh, contains("year"), passage_type) %>% 
  group_by(run_index) %>%
  mutate(aoh_lag = lag(aoh)) %>% 
  # filter(!is.na(aoh_lag)) %>%
  ungroup()
...
tsts %>% filter(run_index == 4233) %>%
  gg_lag(aoh)

tsts %>% 
  filter(run_index == 4233) %>%
  PACF(aoh) %>% 
  autoplot()
...
ggsave(
  aoh_lm_dw_test %>% 
  left_join(select(aoh_trends, aoh_type:obligate_type), by = c("run_index", "aoh_type")) %>%
    filter(
      aoh_type == "crop_abn_iucn",
...
```{r translate-aoh-to-percentage-points}
aoh %>% filter(aoh_type == aoh_type_, 
               passage_type == "exclude_passage",
               str_detect(binomial, "Dryobates"),
               # run_index %in% c(4482:4485),
               site == "wisconsin"
                     ) %>%
  filter() %>%
  ggplot(aes(y = aoh, x = year)) + geom_line()
...
aoh %>% filter(aoh_type == aoh_type_, 
  ...
  select(year, aoh) %>%
  mutate(p_change = (aoh - lag(aoh) )/ lag(aoh),
         m1 = RcppRoll::roll_mean(aoh, n = 1, align = "right", fill = NA),
         m2 = RcppRoll::roll_mean(aoh, n = 2, align = "right", fill = NA),
         m3 = RcppRoll::roll_mean(aoh, n = 3, align = "right", fill = NA),
         m4 = RcppRoll::roll_mean(aoh, n = 4, align = "right", fill = NA),
         m5 = RcppRoll::roll_mean(aoh, n = 5, align = "right", fill = NA),
         m6 = RcppRoll::roll_mean(aoh, n = 6, align = "right", fill = NA),
         m7 = RcppRoll::roll_mean(aoh, n = 7, align = "right", fill = NA),
         ) %>%
  pivot_longer(cols = contains("m"), names_to = "lag", names_prefix = "m", values_to = "aoh_lag") %>%
  mutate(lag = as.integer(lag)) %>%
  # select(aoh) %>%
  # select(p_change) %>% drop_na() %>% pacf()
  ggplot() + 
    geom_line(aes(y = aoh_lag, x = year, col = lag, group = lag))
...
# ----------------------------------------------------------- #
# -- NW -- comparing observed vs. potential
# ----------------------------------------------------------- #



# ----------------------------------------------------------- #
# -- NW -- calculation comparisons
# ----------------------------------------------------------- #
```
...
gg_aoh_sp_obs_wisc_crop_plus1 <- gg_aoh_sp_obs_wisc_crop + 
    geom_line(data = aoh %>%
                filter(
                  binomial == "Sturnella magna",
...
gg_aoh_sp_obs_wisc_crop_plus2 <- gg_aoh_sp_obs_wisc_crop_plus1 + 
    geom_line(data = aoh %>%
                filter(
...
gg_aoh_sp_obs_wisc_crop_plus3 <- gg_aoh_sp_obs_wisc_crop_plus2 + 
    geom_line(data = aoh %>%
                filter(
...
ggsave(aoh_all_types_by_site_tmp,
       filename = paste0(p_output, "plots/aoh/", 
...
# overall stacked:
# -----
gg_tmp_total_b_feols_nw <- aoh_feols_trends_by_sp %>%
    filter(passage_type == "exclude_passage",
         vert_class != "amp", 
         !(vert_class == "bird" & mature_forest_obl > 0.5),
         aoh_type == "crop_abn_iucn",
...
```{r testing-difference}
aoh_l$binomial %>% unique() %>% length()
aoh$binomial %>% unique() %>% length()
...
tttt %>%
  ...
  summarise(n_sp = n()) %>% ungroup() %>%
  group_by(site, vert_class) %>% mutate(n_sp_tot_vert = sum(n_sp)) %>% ungroup() %>%
  group_by(site) %>% mutate(n_sp_total = sum(n_sp)) %>% ungroup() %>%
  mutate(p = 100 * n_sp / n_sp_tot_vert) %>%
  arrange(n_sp_total, site, vert_class, threatened, mature, n_sp) %>% 
  select(site, vert_class, threatened, mature, n_sp, p, n_sp_tot_vert, everything()) %>%
  print(n=70)
 ...
 tttt %>%
  ...
  scale_alpha_manual(values = c(0.95, 0.15, 1), labels = c("no" = "No", "yes" = "Yes")) +
  labs(x = "Number of Species with AOH", y = NULL,
       fill = "Vert. Class", alpha = "Mature For. Obl.") +
...
# previous version:
aoh_species_list_by_site0# <- aoh_species_list_by_site
aoh_species_list_by_site0

species_attributes

aoh_species_list_by_site <-
  # aoh_l %>% # replaced with aoh 8/10/22 in order to exclude passage areas and filter to species with non-zero aoh
  aoh %>% 
  filter(
    passage_type == "exclude_passage",
    aoh > 0,
