...
#### START HERE ####
# add these n_unique_obs == 1 species below, and run the code in the following three chunks again
...
aoh_change_df$aoh_type %>% unique()
# set up temp files:

aoh_change_df_tmp <- 
  aoh_change_df %>% 
  filter(est_type == "estimate", # exclude high and low confidence estimates
         aoh_type == "crop_abn_iucn", 
...
# --------------------------------------------------------------- #
aoh_start_end_l_tmp_plots <- aoh_start_end_l %>%
...
lapply(c("TRUE", "FALSE"), function (constrain_) {
  ...
  ggsave(
    ...
    filename = paste0(p_output, "plots/aoh/effect_size/", "hist_aoh_ratio_obs_",
                      {if(constrain_) "constrained_" else "x"},
                      i, ".pdf"),
    width = 7, height = 8.5, units = "in")
    })
  })
...
# --------------------------------------------------------------- #
names(aoh_start_end_l_tmp_plots)
i
# apply across multiple window sizes:
# lapply(c(1, 5, 10, 15), function(i) {
#   ggsave(
  aoh_start_end_l_tmp_plots %>% 
    filter(window_size == 1) %>%
    ggplot() + 
    geom_histogram(aes(x = abs_change_as_prop_site_area, fill = trend, group = trend), bins = 100, col = "black") +
    geom_vline(data = aoh_start_end_l_tmp_plots %>%
                 filter(window_size == 1, trend != "no trend") %>%
                 group_by(trend, site) %>%
                 summarise(n = n(), mean = mean(abs_change_as_prop_site_area), 
                           median = median(abs_change_as_prop_site_area)) %>%
                 pivot_longer(cols = c("mean", "median"), names_to = "Statistic") %>% ungroup(),
               mapping = aes(xintercept = value, linetype = Statistic, col = trend), size = 0.75) +
    theme_classic() +
    scale_fill_manual(name = "Trend", labels = palette_labels[c(1, 3, 6)], values = palette_du_jour[c(1, 3, 6)]) + 
    scale_color_manual(name = "Trend", labels = palette_labels[c(1, 3, 6)], values = palette_du_jour[c(1, 3, 6)]) + 
    labs(x = "Change in observed AOH as a proportion of site area", y = "Count", 
         caption = paste0("Comparing mean values in first and last ", i, "years")) + 
    theme(legend.position = "bottom", strip.text.y.right = element_text(size = 8, angle = 0, hjust = 0, vjust = 0)) +
    facet_grid(site ~ ., scales = "free", labeller = as_labeller(fancy_labels))
#   ,
#   
#   filename = paste0(p_output, "plots/aoh/effect_size/", "hist_aoh_change_as_prop_site_area_obs_", i, "x.pdf"),
#   width = 7, height = 8.5, units = "in")
#   
# })
...
aoh_feols_trends %>%
  filter(aoh_type == "crop_abn_iucn",
         passage_type == "exclude_passage",
         mature_forest_obl < 0.5) %>%
  ggplot(aes(x = estimate, fill = trend)) + 
  geom_histogram() + 
  facet_wrap(vars(site), scales = "free")
...
```{r explore-negative-ratio-winner-species}
aoh_change_df_tmp %>%
...
# gg_aoh_trend_by_vert_aoh_type <-
  aoh_feols_trends_by_sp %>% # new version, accounting for temporal autocorrelation via NW estimator
  filter(
