...
common_names %>%
  group_by(binomial) %>%
  summarise(names = str_c(common_name, collapse = "; "))
...
species_to_remove <- lapply(1:11, function(i){
  site_habitats <- sort(filter(jung_hab_type_area_df, site == site_df$site[i])$code)
...
# -----------
# extract a list of all siutable habitats for each species
# -----------
habitat_prefs %>%
  filter(suitability == "Suitable") %>%
  # select(binomial, code, name) %>%
  mutate(combined = paste(code, name)) %>%
  arrange(code) %>%
  group_by(binomial) %>%
  summarise(suitable_hab = str_c(combined, collapse = "; "))

# -----------
habitat_details
habitat_prefs

habitat_details[1, ]$habitat
...
tic()
abn_lc <- lapply(c(7,11), function(i) {
  # make sure to restrict lc to only 1987:2017 (Nebraska, Wisconsin)
  tmp <- abn_mask[[i]] + lc[[i]][[paste0("y", 1987:2017)]]
