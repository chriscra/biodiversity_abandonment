...
This document houses code to:
1. Load, manipulate, and model Area of Habitat (AOH) data produced on Princeton's High Performance Computing (HPC) Clusters (see "cluster/aoh.R")
2. Model AOH as a function of time while accounting for temporal autocorrelation (after testing multiple approaches), and extract AOH trends and effect sizes.
...
# 1. Load and Process AOH

```{r set-labels-and-options}
...
```{r load-all-aoh}
# load all raw aoh results: ("aoh_l")
...
# number of species per class per seasonality code
habitat_prefs %>%
...
# this code simply adds the area in specific habitats for each species after filtering.
aoh_filter # load in previous chunk
...
# ------------ #
# combine with relevant species attribute columns from aoh_l
aoh <- 
  aoh %>% 
  left_join(
    aoh_l %>% 
...
write_csv(final_species_list, file = paste0(p_derived, "/final_species_list.csv"))



# ------------------ #
# Number of species in each calculation
...
Start with a very basic OLS regression of AOH as a function of time.
...
# prelim model
# note that year0 is created in chunk {r **calculate-aoh} above, and represents the first year with aoh data for that species in that calculation.
...
## Temporal Autocorrelation: test and account for it 

After examining residuals, it is clear that our trends show signs of temporal autocorrelation (this is not surprising!).
Our final approach to deal with temporal autocorrelation was to use the Newey-West estimator for standard errors, which accounts for temporal autocorrelation.

We also tested a few other approaches for accounting for temporal autocorrelation:

Alt. Approach 1: including term for AOH in the prior year (this is often called a "first order auto-regression (AR1)" approach).
Alt. Approach 2: subsample the data to only include every 3 years (or every 5 years, etc.)
...
### Newey-West standard errors (Final Approach)

Newey-West standard errors, which account for autocorrelation, are generally approved of by the time-series econometrics universe (Newey & West, 1987, https://www.jstor.org/stable/1913610)
We implement this approach using the feols() function in the {fixest} package, as follows:
feols(aoh ~ year0, data = df, vcov = vcov_NW(time = ~year0))

```{r feols-expl}
set.seed(1)
library(fixest) # this package includes the function below feols() which allows for the use of Newey-West 
library(tidyverse)
library(broom)
...
```{r **feols}
# calculate an OLS regression using the Newey-West estimator to calculate standard errors, using the package {fixest}
  ...
  aoh_types,
       function(aoh_type_) {
         aoh_tmp <- aoh %>% 
           ...
           # group_by(run_index) %>% # no longer necessary - see above, "{r **calculate-aoh}"
           # mutate(n_obs = n()) %>% 
           # ungroup()
...
### Alternative Approaches for handling Temporal Autocorrelation

#### Approach 1: Auto-regressive (AR1), including term for AOH in the prior year
...
#### Approach 2: subsample to every 3 or 5 years
...
Observed effect sizes are used in all models except models on model slope values.

```{r observed-change-in-aoh-by-window-size}
# Calculate observed change in AOH by comparing the mean AOH in the last X years with the mean AOH in the first X years of the time series. 
# We compare for a variety of window lengths (1, 5, 10, and 15 years) for calculating the initial and final AOH values. 
...
aoh_change_df <-
  ...
  mutate(time_range = end_year - start_year,
         ...
         # abs_change_per_yr = abs_change / time_range, # dropped Sep. 2024, doesn't add any value and is duplicative of slope.
         abs_change_as_prop_site_area = abs_change / total_site_area_ha_2017,
...
# When using a threshold of 10-20% to indicate increases or decreases, trends match for about 95% of species.
...
```{r seff}
# Assessing the efficiency of various job lengths and memory allocations on the computing cluster.
