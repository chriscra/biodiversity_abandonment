...
species_ranges <- filter(vert_sites, vert_class != "gard")
...
# site_jung_l2_30 <- 
  ...
  list.files(paste0(p_derived, "site_jung"), full.names = TRUE) %>% 
    grep("l2_30.tif", ., value = TRUE), 
  function(i) rast(i)
)
names(habitat_map) <- site_df$site

# load elevation raster
elevation_t <- lapply(1:11, function(i) {
  rast(paste0(p_derived, "elevation/", site_df$site[i], "_srtm_crop.tif"))
  }
  )
names(elevation_t) <- site_df$site
...
IUCN_crosswalk %>% 
  left_join(iucnHabitatTranslator, by = "map_code") %>% .$code %>% unique() %>% sort()
...
species_list[, 3]
results <- tibble(species = species_list[, "binomial"],
                  IUCN_AOO = NA,
                  IUCN_AOH = NA)
...
# results$IUCN_AOO[i] <- expanse(x2, unit = "km") # in km2
...
print(i)
write.csv(results,"~/Dropbox/PostDoc4_Princeton/Species Traits/FutureAOH.csv")
}
```
