...
sp_w_mult <- 
  ...
  filter(site == site_df$site[9]) %>%
    select(site:seasonal, -id_no, CommonName) %>%
    st_drop_geometry() %>%
    group_by(binomial) %>% summarise(n = n()) %>% arrange(desc(n)) %>% .[1:4,1] %>% .$binomial

species_ranges %>%
  filter(site == site_df$site[9],
         binomial %in% sp_w_mult[2]) %>%
    select(site:seasonal, -id_no, CommonName) %>%
  st_cast() %>%
  select(seasonal) %>%
    plot()
...
range_t <-
  ...
  # select(seasonal) %>%
    # plot()
      fasterize(.,
                raster::raster(resolution = terra::res(elevation_map[[9]]),
                               ext = raster::extent(terra::ext(elevation_map[[9]])[1:4])),
                field = "seasonal"
      ) %>% terra::rast(.)
...
jung_hab_type_area_df %>%
  select(habitat_type, lc, Coarse_Name, IUCNLevel, code) %>%
  unique() %>% arrange(habitat_type) %>% print(n = 40)
```
...
```{r species_run_index}
species_list %>%
  left_join(select(habitat_prefs, ))
...
max_age_mask <- lapply(1:11, function(i) {
  max_age_mask_tmp <- 
    classify(
      ...
      othersNA = TRUE,
      include.lowest = TRUE, right = TRUE,
      filename = paste0("/Users/christophercrawford/work/projects/abandonment_trajectories/data_derived/max_age/max_age_mask/",
                        site_df$site[i], "_max_age_5_30_mask.tif"),
...
max_age_mask <- lapply(1:11, function(i){
  rast(paste0("/Users/christophercrawford/work/projects/abandonment_trajectories/data_derived/max_age/max_age_mask/", site_df$site[i], "_max_age_5_30_mask.tif"))
})
...
max_abn_lcc <- lapply(1:11, function(i) {
  ...
  writeRaster(tmp, 
              filename = paste0(p_derived, "abn_lcc/",
                        site_df$site[i], "_max_abn_lcc.tif"), 
              overwrite = TRUE,
              names = paste0("y", 1987:2017))
  cat(site_df$site[i], "done!", fill = TRUE)
...
```{r load-cluster-results-full-lc}
run_time <- "_2021_12_12" # "_2021_12_05"

aoh_lc_df <- lapply(
  list.files(paste0(p_derived, "aoh"), full.names = T) %>% 
...
aoh_abn <- lapply(1:18, function(i) {
  read_csv(paste0(p_derived, "aoh/aoh_abn_lc_tmp", "_2021_12_13", "_c", i, ".csv"))
  }
  ) %>% 
  bind_rows() %>% 
  left_join(select(habitat_details, binomial, redlistCategory), by = "binomial") %>%
...
aoh_max_abn <- lapply(1:18, function(i) {
  read_csv(paste0(p_derived, "aoh/aoh_max_abn_lc_tmp", "_2021_12_13", "_c", i, ".csv"))
  }
  ) %>% 
  bind_rows() %>% 
  left_join(select(habitat_details, binomial, redlistCategory), by = "binomial") %>%
...
# prelim model
tmp_lm <- lm(adj_IUCN_aoh_ha ~ year, data = filter(dft, binomial == sp_to_plot, site == "belarus"))
tidy(tmp_lm)
glance(tmp_lm)
augment(tmp_lm)


ggplot(data = filter(dft, binomial == sp_to_plot, site == "belarus"), 
...
dft <- dft %>%
  left_join(run_indices, by = c("binomial", "site", "vert_class"))
...
ggsave(gg_aoh_percent_change_hist,
       filename = paste0(p_output, "plots/aoh/", "aoh_percent_change_hist.pdf"),
...
trend_df %>% arrange(redlistCategory, vert_class) %>% print(n = 31)

trend_by_site <- aoh_trends %>% 
  left_join(dft %>% 
              select(site, run_index, binomial, redlistCategory, vert_class) %>%
              unique(), 
            by = "run_index") %>%
...
ggsave(gg_aoh_trend_by_cat,
       filename = paste0(p_output, "plots/aoh/", "aoh_trend_by_iucn_cat.pdf"),
...
ggsave(gg_aoh_trend_by_site,
       filename = paste0(p_output, "plots/aoh/", "aoh_trend_by_site.pdf"),
       width = 8, height = 5, units = "in")
...
ggsave(gg_aoh_abn_percent_change_hist,
       filename = paste0(p_output, "plots/aoh/", "aoh_abn_percent_change_hist.pdf"),
       width = 8, height = 5, units = "in")
...
ggsave(gg_aoh_abn_trend_by_cat,
       filename = paste0(p_output, "plots/aoh/", "aoh_abn_trend_by_iucn_cat.pdf"),
       width = 7, height = 5, units = "in")
...
ggsave(gg_aoh_abn_trend_by_site,
       filename = paste0(p_output, "plots/aoh/", "aoh_abn_trend_by_site.pdf"),
       width = 8, height = 5, units = "in")
...
ggsave(gg_aoh_max_abn_percent_change_hist,
       filename = paste0(p_output, "plots/aoh/", "aoh_max_abn_percent_change_hist.pdf"),
       width = 8, height = 5, units = "in")
...
ggsave(gg_aoh_max_abn_trend_by_cat,
       filename = paste0(p_output, "plots/aoh/", "aoh_max_abn_trend_by_iucn_cat.pdf"),
       width = 7, height = 5, units = "in")
...
ggsave(gg_aoh_max_abn_trend_by_site,
       filename = paste0(p_output, "plots/aoh/", "aoh_max_abn_trend_by_site.pdf"),
       width = 8, height = 5, units = "in")
...
gg_aoh_example1 <- 
  aoh_combo %>%
  filter(site == site_df$site[9], binomial == sp_name) %>%
  select(site:year, adj_IUCN_aoh_ha, redlistCategory, vert_class, type) %>% unique() %>%
  pivot_wider(id_cols = c("site", "binomial", "redlistCategory", "vert_class", "type", "year"), 
              names_from = type, values_from = adj_IUCN_aoh_ha) %>%
  mutate(diff = lc - abn, diff_max = lc - max_abn) %>%
  pivot_longer(cols = c("abn", "max_abn", "lc", "diff", "diff_max"),
  ...
  scale_color_manual(name = "Type",
                     labels = c("diff_max" = "Difference",
                                "lc" = "All land cover",
                                "max_abn" = "Only abandonment"),
                     values = gg_color_hue(3))
  

# save plot
ggsave(gg_aoh_example1,
       filename = paste0(p_output, "plots/aoh/", "aoh_example_species1.pdf"),
...
aoh_combo %>%
  filter(site == site_df$site[9], binomial == sp_name) %>%
  select(site:year, adj_IUCN_aoh_ha, redlistCategory, vert_class, type) %>%
  pivot_wider(id_cols = c("site", "binomial", "redlistCategory", "vert_class", "type", "year"), 
              names_from = type, values_from = adj_IUCN_aoh_ha)

gg_aoh_example2 <-
  aoh_combo %>%
  filter(site == site_df$site[9], binomial == sp_name2) %>%
  select(site:year, adj_IUCN_aoh_ha, redlistCategory, vert_class, type) %>% unique() %>%
  pivot_wider(id_cols = c("site", "binomial", "redlistCategory", "vert_class", "type", "year"), 
              names_from = type, values_from = adj_IUCN_aoh_ha) %>%
  mutate(diff = lc - abn, diff_max = lc - max_abn) %>%
...
habitat_details %>% filter(binomial == sp_name) %>% .$habitat
habitat_prefs %>% filter(binomial == sp_name) %>% select(code, name)
habitat_prefs %>% filter(binomial == sp_name2) %>% select(code, name)
  

# save plot
ggsave(gg_aoh_example2,
       filename = paste0(p_output, "plots/aoh/", "aoh_example_species2.pdf"),
       width = 8, height = 6, units = "in")
...
aoh_combo %>%
  ...
  pivot_wider(id_cols = c("site", "binomial", "redlistCategory", "vert_class", "type", "year"), 
              names_from = type, values_from = adj_IUCN_aoh_ha) %>%
  mutate(diff = lc - abn) %>%
  pivot_longer(cols = c("abn", "lc", "diff"),
               names_to = "type", values_to = "aoh") %>%
  # print(n = 80)
