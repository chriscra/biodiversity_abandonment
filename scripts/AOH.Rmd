...
aoh_feols_trends %>%
  ...
  select(aoh_type, vert_class, binomial, passage_type, site, redlistCategory, obligate_type) %>%
  pivot_wider(id_cols = c(aoh_type, vert_class, binomial, passage_type, redlistCategory, obligate_type),
  ...
  left_join(aoh_feols_trends_by_sp %>% 
              select(aoh_type:passage_type, overall_trend, range_size_quantile, common_names),
            by = c("aoh_type", "vert_class", "binomial", "passage_type")) %>%
    select(vert_class, binomial, overall_trend, sites, redlistCategory, obligate_type, range_size_quantile, common_names) %>%
  filter(obligate_type == "not_obligate")


aoh_feols_trends_by_sp %>% select(aoh_type:passage_type, overall_trend, range_size_quantile, common_names)
...
lapply(
  ...
  gg_aoh_by_site_prop_tmp <-
    aoh_ms_tmp_trends %>%
      ...
      facet_wrap(vars(site), scales = "free", nrow = 4, ncol = 3,
                 labeller = as_labeller(fancy_labels)) +
      theme(legend.position = c(0.835, 0.0985), legend.background = element_blank(),
            legend.text = element_text(size = 7),
...
aoh_ms_tmp_trends %>%
      ...
      mutate(vert_class = case_when(vert_class == "bird" ~ "Birds", vert_class == "mam" ~ "Mammals"),
             vert_class = paste0(vert_class, "\n(n = ", total_sp, ")"),
             # trend = fct_relevel(trend, names(palette_du_jour))
             ) %>%
      arrange(vert_class, aoh_type) %>%
      ggplot(mapping = aes(x = vert_class, y = n_sp/total_sp,
                           # label = total_sp, 
...
aoh %>%
      ...
      left_join(aoh_feols_trends) %>% 
  left_join(area_summary_df, by = "site") %>%
  mutate(new_var = 
           100 * aoh / total_crop_extent_ha
           # 100 * aoh/total_site_area_ha_2017
         ) %>%
      ggplot(aes(x = year, y = new_var, col = trend, group = binomial)) +
      geom_line(alpha = 0.15) + 
      theme_classic() + 
      scale_color_manual(labels = palette_labels[c(1, 3, 6)],
                         values = palette_du_jour[c(1, 3, 6)]) +
...
lapply(
  ...
  ggsave(
    aoh_start_end_l_tmp_plots %>% 
      filter(window_size == i) %>%
      # filter(site %in% site_df$site[1:6]) %>%
      # filter(site %in% site_df$site[c(1, 7:11)]) %>%
      ggplot() + 
      geom_histogram(aes(x = ratio, fill = trend, group = trend), bins = 100, col = "black") +
      ...
      labs(
        ...
        # x = expression(Ratio~of~ending~AOH~to~starting~AOH~(shown~on~a~log["10"]~scale)), 
        y = "Number of Species", 
...
```{r *fig3}
# figure 3, developed in the chunk above "plot-aoh-change-obs", producing files named:
# "plots/aoh/effect_size/", "hist_aoh_change_percent_site_obs_by_trend_", window_size_, ".pdf"
# now, produced as figure_3
# ------------------------------------------------------ #
# AOH Change as a percent of site area, by trend
# ------------------------------------------------------ #
lapply(c("pdf", "png"), function(file_type_) {
  ...
  aoh_start_end_l_tmp_plots %>% 
    ...
    geom_histogram(aes(x = abs_change_percent_site, fill = trend, group = trend), 
                   ...
                   bins = 50, col = "black") +
    geom_vline(data = aoh_start_end_l_tmp_plots %>%
                 filter(window_size == 5, #trend != "no trend"
                        ) %>%
                 group_by(trend, 
                          vert_class
                          ) %>%
...
gg_fpo_overall_crop <-
  ...
  ggplot(mapping = aes(
    ...
    fill = 
    overall_trend %>% fct_relevel(names(palette_du_jour)[c(1, 2, 6, 5, 3, 4)])
    # group = aoh_type
         )) +
  geom_col(color = "black", size = 0.25, position = position_stack(reverse = TRUE)) +
  theme_classic() +
 scale_x_discrete(labels = palette_labels) +
  scale_y_continuous(n.breaks = 10) +
  labs(x = "AOH Trend", y = "Number of Species") +
...
gg_fpo_overall_crop_w_full <-
  ...
  ggplot(mapping = aes(x = trend_direction, 
                       y = n_sp,
                       fill = overall_trend %>% 
                         fct_relevel(names(palette_du_jour)[c(1, 2, 6, 5, 3, 4)]),
                       # group = aoh_type
         )) +
  geom_col(color = "black", size = 0.25, position = position_stack(reverse = TRUE)) +
  theme_classic() +
...
ggsave(plot_grid(gg_tmp_total_b, gg_tmp_total_m, 
          ...
          ncol = 3),
       filename = paste0(p_output, "plots/aoh/", 
                         "fpo_overall_crop_abn_stacked", aoh_run_date,
                         ".pdf"),
       width = 4.25, height = 4, units = "in")
...
plot_grid(gg_tmp_total_b_obs_pot, 
          gg_tmp_total_m_obs_pot, 
          ggpubr::get_legend(gg_fpo_overall_crop + theme(legend.position = "right")),
          rel_widths = c(1, 1, 0.7),
          ncol = 3)

ggsave(plot_grid(gg_tmp_total_b_obs_pot, 
                 gg_tmp_total_m_obs_pot, 
          ggpubr::get_legend(gg_fpo_overall_crop + theme(legend.position = "right")),
          rel_widths = c(1, 1, 0.6),
          ncol = 3),
