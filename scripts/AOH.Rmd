...
aoh_types <- aoh_type_df$label[c(4:8)]
aoh_type_ <- aoh_types[3]

aoh_l <- lapply(aoh_types, 
  function(aoh_type_) {
  lapply(1:21, 
         function(i) {
    read_csv(paste0(p_derived, "aoh/aoh_tmp_", aoh_type_,
                    ifelse(grepl("max", aoh_type_), 
                           ifelse(grepl("potential", aoh_type_),
...
aoh_trends_by_sp <- 
  ...
  mutate(
    ...
    aoh_type = fct_relevel(aoh_type, aoh_types[c(3, 5, 1, 2, 4)])
    )

aoh_types
aoh_trends_by_sp %>% 
  select(aoh_type) %>% 
  mutate(aoh_type = as_factor(aoh_type)) %>% 
  .$aoh_type %>% 
  levels()
...
ggsave(TBD,
       filename = paste0(p_output, "plots/aoh/", "sdf", aoh_run_date, "_", j,
                         "_", mature_opt,
                         "_", passage_opt,
                         ".pdf"),
