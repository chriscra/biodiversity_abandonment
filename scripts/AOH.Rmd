...
df_tmp <-
  ...
  select(aoh_type, vert_class, binomial, site, contains("estimate"), passage_type) %>%
  left_join(aoh_start_years) %>%
...
gg_aoh_trend_lines_by_site <-
  ...
  guides(color = guide_legend(override.aes = list(alpha = 1, size = 1.5, order = 1)),
         # linetype = guide_legend(direction = "vertical", byrow = FALSE, 
...
ggplotly(gg_aoh_trend_lines_by_site)
```
...
# see above, in "plot overlapping lines":
