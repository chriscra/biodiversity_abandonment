...
aoh_start_years <-
  aoh %>%
  group_by(aoh_type, vert_class, site, binomial) %>%
...
df_tmp <-
  ...
  select(aoh_type, vert_class, binomial, site, contains("estimate")) %>% ungroup() %>%
  left_join(aoh_start_years) %>%
  mutate(start_year = start_year - 1986, end_year = end_year - 1986) %>%
  left_join(aoh_trends_by_sp %>% 
              select(aoh_type, vert_class, binomial, passage_type,
                     overall_trend, trend_direction)) %>%
  left_join(aoh_trends %>% select(aoh_type, vert_class, binomial, passage_type, trend)) %>%
...
gg_aoh_trend_lines_by_site <- 
  ...
  ggplot(aes(x = year + 1986, y = est/10^6, col = trend,
             group = interaction(binomial, site))) +
...
ggsave(gg_aoh_trend_lines_by_site,
       filename = paste0(p_output, "plots/aoh/", "trend_lines_by_site_", i,  aoh_run_date,
                         ".pdf"),
...
ggsave(gg_trends_stack_percent_no_mature,
       filename = paste0(p_output, "plots/aoh/", "trend_summary_stacked_by_mature_obl", aoh_run_date, ".pdf"),
...
```{r mature-forest-obl}
gg_trend_summary_by_mature_for_obl <- 
  aoh_trends_by_sp %>%
  filter(passage_type == passage_opt,
         vert_class != "amp", 
         !aoh_type %in% c("abn_iucn", "potential_abn_iucn"),
 ...
 scale_x_discrete(labels = palette_labels) +
  ...
  theme(legend.position = "right",
            axis.text.x = element_text(angle = 340, vjust = 1, hjust = 0)) +
  facet_grid(
    col = vars(mature_forest_obl), rows = vars(aoh_type),
    scales = "free_y", 
    labeller = as_labeller(c(aoh_type_labels, 
...
ggsave(gg_trend_summary_by_mature_for_obl,
       filename = paste0(p_output, "plots/aoh/", "trend_summary_by_mature_for_obl", aoh_run_date,
                         ".pdf"),
       width = 6, height = 5, units = "in")
```
...
# ----------------------------------------------------------------------- #
for (threat_status in c("Threatened", "Not Threatened")) {
  for(aoh_type_ in aoh_types[c(6, 3, 1)]) {
gg_trend_summary_by_iucn_status_lumped_by_site <-
  ...
  filter(threatened == threat_status) %>%
  ggplot(data = .,
       mapping = aes(
         # x = threatened, y = n_sp,
 ...
 scale_x_discrete(labels = c("mam" = "Mammals", "bird" = "Birds")) +
  # scale_x_continuous(labels = c("0", "0.25", "0.5", "0.75", "1")) +
  labs(y = "Number of Species", x = paste0(threat_status, " Species")) +
  # labs(x = "AOH Trend", y = "Proportion of Species") +
...
ggsave(gg_trend_summary_by_iucn_status_lumped_by_site,
       filename = paste0(p_output, "plots/aoh/", "trend_summary_by_IUCN_status_by_site_", 
                         aoh_type_, "_",
                         threat_status, aoh_run_date,
                         ".pdf"),
       width = 6, height = 5, units = "in")
}
}
...
ggsave(gg_trend_summary_by_site,
       filename = paste0(p_output, "plots/aoh/", "trend_summary_by_site_", i, aoh_run_date,
                         ".pdf"),
...
ggsave(gg_trend_summary_by_site_mat_obl,
       filename = paste0(p_output, "plots/aoh/", "trend_summary_by_site_mat_obl_", i, aoh_run_date,
                         ".pdf"),
...
ggsave(gg_trend_summary_by_site_stacked_3,
       filename = paste0(p_output, "plots/aoh/", "trend_summary_by_site_stacked_3", aoh_run_date,
                         ".pdf"),
...
ggsave(gg_trends_obs_v_pot_overall,
       filename = paste0(p_output, "plots/aoh/", "trend_summary_obs_v_pot_overall", aoh_run_date, ".pdf"),
