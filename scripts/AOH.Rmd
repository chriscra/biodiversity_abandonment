...
aoh_l %>% group_by(aoh_type, vert_class, site) %>%
...
```{r check-species-for-divering-trends}
# Classify species based on variation in trend across sites:
# 1. Always winners
# 2. Always losers
# 3. Context dependent

# --------
# Distill to trends for each species

aoh_trends %>%
  filter(passage_type == "exclude_passage", mature_forest_obl < 0.5) %>%
  group_by(binomial, aoh_type, 
           # passage_type, 
           # mature_forest_obl = mature_forest_obl > 0.5 # group by an expression
           ) %>%
  summarise(#trend = unique(trend),
            trends = str_c(unique(trend), collapse = "; "))

# sites with each trend for each species:
aoh_trends %>%
  filter(passage_type == "exclude_passage", mature_forest_obl < 0.5) %>%
  select(aoh_type, vert_class, binomial, trend, passage_type, site) %>%
  group_by(binomial, aoh_type, trend) %>%
  summarise(n_site = str_c(unique(site), collapse = "; "))
...
# sites where each species has a specific trend.
aoh_trends %>%
  filter(passage_type == "exclude_passage"#, mature_forest_obl < 0.5
         ) %>%
  select(aoh_type, vert_class, binomial, trend, passage_type, site) %>%
  group_by(binomial, aoh_type, trend) %>%
  summarise(sites = str_c(unique(site), collapse = "; ")) %>%
  # ungroup() %>% select(binomial) %>% unique() 
  filter()

# ---------------------------------------------------- #
# create data.frame with a single row for each species in each aoh type, with the number of trends and the specific trend types they experience:
# ---------------------------------------------------- #
...
aoh_trends_by_sp %>%
  ...
  select(n_trends, trend_types, overall_trend, trend_direction, trend_consistency) %>% unique()




aoh_trends_by_sp %>% select(n_trends, trend_types, overall_trend) %>% unique()

filter(aoh_trends, binomial == filter(aoh_trends_by_sp, is.na(overall_trend))$binomial[3])


# count number of each trend type:
aoh_trends_by_sp %>%
  mutate(
    trend_direction = case_when(
      grepl("gain", overall_trend) ~ "gain",
    ...
    trend_consistency = case_when(
      ...
      fct_relevel(c("consistent", "weak", "opposite"))
    ) %>%
  ...
  facet_grid(col=vars(vert_class),
             rows = vars(aoh_type), 
             scales = "free")
...
for (i in 1:4) {
  ...
  passage_opt <- filter(options_df, run == i)$passage_opt
  mature_opt <- filter(options_df, run == i)$mature_opt
