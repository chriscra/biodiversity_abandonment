...
# Tomorrow
```{r}
aoh_31_dfc %>% print(n = 100)
aoh_31_dfc %>% 
  filter(core_index == 1)

aoh_31_dfc %>% filter(site == "mato_grosso") %>%
  .$adj_IUCN_aoh_ha %>%
  summary()

aoh_31_dfc %>%
  select(site, binomial, year, adj_IUCN_aoh_ha, core_index) %>%
  unique() %>%
  group_by(core_index) %>%
  summarise(runs = n()) #%>% .$runs %>% sum()

species_list %>% 
  group_by(core_index, site) %>%
  summarise(runs = n())# %>% .$runs %>% sum()
...
species_list %>% select(site,  binomial) %>% 
  unique()

calc_cores
species_list %>% 
  filter(site == "orenburg") %>% 
  select(core_index) %>% unique()
...
aoh_type <- aoh_type_df$label[3]
aoh_type_df

aoh_types <- aoh_type_df$label[c(1, 3, 4, 6)]

aoh_l <- lapply(aoh_types, 
  function(aoh_type) {
  lapply(1:21, 
         function(i) {
    read_csv(paste0(p_derived, "aoh/aoh_tmp_", aoh_type,
                    ifelse(grepl("max", aoh_type), "_2022_04_08", aoh_run_date),
                    "_c", i, ".csv"))
    }
    ) %>% 
    bind_rows() %>% 
    mutate(aoh_type = aoh_type) %>%
    left_join(iucn_status, by = "binomial") %>%
    left_join(species_list, by = c("site", "binomial")) %>%
    left_join(
  ...
  select(aoh_type, vert_class, site, binomial, year, map_code, lc, season = seasonality, 
         area, aoh, IUCN_aoh_ha, adj_IUCN_aoh_ha,mature_forest_obl, redlistCategory, area_ha, adj_area_ha, everything())
...
habitat_age_req_coded %>% 
  mutate(mature_forest_obl = as.numeric(mature_forest_obl)) %>%
  select(binomial, mature_forest_obl, water_obl, coder, common_names)

species_list$vert_class %>% unique()

obj_size(aoh_l)

aoh_l %>% select(aoh_type) %>% unique() %>% .$aoh_type %>% grepl("lc", .)

aoh_l %>% names()

aoh_l %>%
  mutate(aoh = case_when(
    grepl("lc", aoh_type) ~ adj_IUCN_aoh_ha,
    grepl("iucn", aoh_type) ~ IUCN_aoh_ha
  )) %>%
  select(aoh_type, contains("aoh")) %>%
  filter(aoh_type == aoh_type_df$label[4])
...
species_ranges %>% filter(seasonal == 4)
species_ranges %>% filter(str_detect(binomial, "Falco peregrinus"))
habitat_prefs %>% filter(str_detect(binomial, "Falco peregrinus"), !is.na(map_code))
  ...
  left_join(
    aoh_l %>%
      filter(
        ...
        binomial == "Bombina bombina", 
        site == "belarus", 
        aoh_type %in% aoh_type_df$label[c(1)],
        # year == 2015,
        season %in% c(1, 2, 3)) %>% # filtering out passage areas:
      group_by(aoh_type, vert_class, site, binomial, year) %>% 
      summarise(
        aoh_no_passage = sum(area, na.rm = TRUE)
                )
...
aoh_distill %>%
  filter(binomial == "Bombina bombina", site == "belarus") %>%
  print(n = 40)
...
# one index for each unique species at each site, in each of the aoh runs
run_indices <- 
  aoh_distill %>%
  arrange(aoh_type, vert_class, site, binomial) %>%
  select(aoh_type, vert_class, site, binomial, common_names, redlistCategory, mature_forest_obl) %>%
  unique() %>% 
...
aoh_distill %>%
  filter(binomial == sp_to_plot, aoh_type %in% aoh_type_df$label[c(3,4)]) %>%
...
# -------------------
tic()
aoh_trends <- lapply(1:length(unique(aoh_distill$run_index)), function(i) {
  aoh_distill %>% 
    filter(run_index == i) %>%
    lm(aoh ~ year0, data = .) %>%
    tidy() %>% 
...
tic()
aoh_trends_no_passage <- lapply(
  # 1:length(unique(aoh_distill$run_index)), 
  filter(aoh_distill, !is.na(aoh_no_passage))$run_index %>% unique() %>% sort(), 
  function(i) {
    lm_tmp <- aoh_distill %>% 
      filter(run_index == i) %>%
      lm(aoh_no_passage ~ year0, data = .) %>%
      tidy() %>% 
      mutate(run_index = i)
...
write_parquet(aoh_trends_no_passage, paste0(p_derived, "aoh_trends_no_passage.parquet"))
...
aoh_trends_no_passage <- read_parquet(
  paste0(p_derived, "aoh_trends_no_passage.parquet"))

aoh_trends_combo <- bind_rows(
  aoh_trends %>% mutate(include_passage = "yes"),
  aoh_trends_no_passage %>% mutate(include_passage = "no")
)
...
aoh_distill %>% 
  ...
  select(run_index) %>% summary()

habitat_age_req_coded %>% 
  filter(binomial == unique(filter(aoh_distill, run_index == i)$binomial))
...
aoh_distill %>% 
  ...
  geom_point() + 
  geom_smooth(method = "lm", se = 0.95) + 
  labs(x = "Year", y = "AOH (ha)", title = unique(filter(aoh_distill, run_index == i)$binomial))
  


# calculate percent changes
aoh_change_df <- 
  aoh_trends_combo %>% #filter(run_index == i) %>%
  select(run_index, include_passage, 
         term, estimate) %>%
  mutate(term = ifelse(term == "(Intercept)", "intercept", "slope")) %>%
  pivot_wider(id_cols = c(run_index, include_passage
                          ), 
...
aoh_trends$run_index %>% unique()
aoh_change_df %>% filter(vert_class != "amp")
...
options_df <- tibble(
  run = 1:4,
  include_passage_areas = c("yes", "yes", "no", "no"),
  filter_out_mature_forest_obl = c("yes", "no", "yes", "no")
)

# i <- 2
for(i in 1:4) {
  
  include_passage_areas <- filter(options_df, run == i)$include_passage_areas
  filter_out_mature_forest_obl <- filter(options_df, run == i)$filter_out_mature_forest_obl
...
gg_aoh_percent_change_hist_tmp <-
  ...
  {if (filter_out_mature_forest_obl == "yes") filter(., mature_forest_obl < 0.5) else .} %>%
  ...
  labs(x = "Percent change in AOH, 1987 to 2017 (%)", y = "Number of species",
       subtitle = filter(aoh_type_df, label == i)$desc,
       caption = paste0(#filter(aoh_type_df, label == i)$desc, 
                        if(filter_out_mature_forest_obl == "yes") {paste0("", "excluding mature forest obligates")} else {""},
                         if(include_passage_areas == "no") {paste0(", ", "excluding passage areas")} else {""})) +
...
ggsave(gg_aoh_percent_change_hist_tmp,
       filename = paste0(p_output, "plots/aoh/", "aoh_percent_change_hist", 
                         aoh_run_date, "_", i , 
                         if(filter_out_mature_forest_obl == "yes") {"_no_mature_forest_obl"} else {""},
                         if(include_passage_areas == "no") {"_no_passage"} else {""},
                         ".pdf"),
       width = 8, height = 5, units = "in")
}
}

aoh_distill %>% filter(run_index == i) %>%
...
trend_by_category <- aoh_trends_combo %>% 
  left_join(run_indices) %>%
  mutate(mature_forest_obl_lgl = mature_forest_obl > 0.5) %>%
  filter(term == "year0",
         p.value < 0.05,
  ...
  group_by(aoh_type, vert_class, 
           # site, 
           redlistCategory,
           include_passage, mature_forest_obl_lgl, 
           trend) %>%
...
# filter_out_mature_forest_obl <- "yes"
...
ggsave(gg_aoh_trend_by_cat_tmp,
       filename = paste0(p_output, "plots/aoh/", "aoh_trend_by_iucn_cat", aoh_run_date, "_", i , 
                         if(filter_out_mature_forest_obl == "yes") {"_no_mature_forest_obl"} else {""},
                         if(include_passage_areas == "no") {"_no_passage"} else {""},
                         ".pdf"),
...
ggsave(gg_aoh_trend_by_site,
       filename = paste0(p_output, "plots/aoh/", "aoh_trend_by_site", aoh_run_date, "_", i , 
                         if(filter_out_mature_forest_obl == "yes") {"_no_mature_forest_obl"} else {""},
                         if(include_passage_areas == "no") {"_no_passage"} else {""},
                         ".pdf"),
...
```{r extracting-trend-abn}


aoh_combo %>%
  filter(site == site_df$site[9], vert_class == "bird") %>%
  select(binomial) %>% unique() %>% print(n = 50)
...
gg_aoh_example1 <-
  ...
  labs(x = "Year", y = "AOH (ha)", 
       title = paste(sp_name, "(Eurasian treecreeper)"),
       subtitle = site_df$site[1]) +
  scale_color_manual(
    name = "Type",
  ...
  # save plot
  ggsave(plot = gg_aoh_example1,
         filename = paste0(p_output, "plots/aoh/", "aoh_example_species1", aoh_run_date, ".pdf"),
...
```{r seff}
# extract run codes:
list.files("scripts/cluster/slurm_out") %>% grep("aoh_hab*", ., value = TRUE) %>%
...
seff <- tibble(
  ...
  mutate(
    ...
    file = 
      ...
      str_subset(array_id),
    label = str_replace(all_slurm_out_files,
...
# Following up from presentation

1. How is 100 and 104 allocated?
2. Species ranges - breeding, passage, etc.
```{r}
