...
(ref:caption-main-aoh-stats-table) Number of bird and mammals with various trends in area of habitat (AOH) across different spatial and temporal calculations of AOH. The number of individual species-site trends are shown in Table \@ref(tab:aoh-stats-table-all-obs).

```{r main-aoh-stats-table, tab.cap = '(ref:caption-main-aoh-stats-table)', echo = FALSE, message = FALSE, eval = TRUE}
# table for MS:
...
aoh_table_tmp2 <- 
  ...
  mutate(win_loss_weak = round((gain + weak_gain) / (loss + weak_loss), (digits = 2))) %>%
...
aoh_table_tmp1 %>%
  ...
  mutate(aoh_type = case_when(
    ...
    aoh_type == "crop_abn_potential_iucn" ~ "Abandonment, w/o recultivation",
    aoh_type == "max_abn_iucn" ~ "Abandonment, incl. habitat loss prior to cultivation",
  ...
  select("AOH Calculation" = aoh_type, "Vert. Class" = vert_class,
         ...
         "Ratio win/loss" = win_loss,
         "Ratio win/loss incl. weak trends" = win_loss_weak) %>%
  flextable() %>% 
  theme_booktabs() %>%
  autofit()

  # knitr::kable(
  #   # col.names = c("AOH Calculation", "Vert. Class",
  #   #               "Gain", "Weak Gain", "Loss", "Weak Loss",
  #   #               "No Trend", "Conflicting", "Ratio win/loss",
  #   #               "Ratio win/loss incl. weak trends"),
  #            align = "r", 
  #            #format = "simple",
  #            # format = my_format,
  #            booktabs = TRUE,
  #            # longtable = TRUE,
  #            caption = "Number of bird and mammals with various trends in area of habitat (AOH) across different spatial and temporal calculations of AOH.") 
```


(ref:caption-global-map-w-wisc) Site locations and example trends of Area of Habitat (AOH) for mammals and birds at one site, Wisconsin, USA, following cropland abandonment. *a*, Locations of our 11 sites from @Yin2020 [map adapted from @Crawford2022]. *b*, trends in AOH (10^6 ha) following cropland abandonment for all birds and mammals occurring in our site in Wisconsin, USA. Trends are colored according to the direction of a species trend in AOH over the course of the time series, with statistically significant increases shown in green, declines in gold, and no trend in black. Bold trend lines highlight species in each category: *c*, House Wren (*Troglodytes aedon*; photo by Tyler Beames, [ML469603381](https://macaulaylibrary.org/asset/469603381)); *d*, Eastern Meadowlark (*Sturnella magna*; photo by Brad Imhoff, [ML225564911](https://macaulaylibrary.org/asset/225564911)); *e*, Downy Woodpecker (*Dryobates pubescens*; photo by Ryan Brady, [ML304722431](https://macaulaylibrary.org/asset/304722431)).
...
(ref:caption-aoh-by-site-crop-abn-prop) Proportion of bird and mammal species with various trends in AOH in abandoned pixels starting from the year of first cultivation and running through 2017 (Calculation 1; see Methods). Proportions are shown for individual sites (with the overall number of birds and mammals in x-axis labels), and the total number of species across all sites is shown at left ("Overall"). Species counts in each trend are shown in Fig. \@ref(fig:aoh-by-site-crop-abn), and the observed and modeled AOH trends for individual species are shown in Figs. \@ref(fig:aoh-sp-obs-crop-abn) and \@ref(fig:aoh-trend-lines-crop-abn).
...
### Magnitude of changes in AOH following abandonment {.unnumbered}

```{r effect-size-stats, include = FALSE, eval = FALSE}
...
aoh_start_end_l_tmp_plots %>% filter(window_size == i)
...
Though there were a higher number of winners, the species that lost habitat on the whole lost more habitat on average than species that gained habitat gained.
See Figs. \@ref(fig:aoh-change-percent-by-trend) and \@ref(fig:aoh-change-percent-by-site).



Key conclusions:

(ref:caption-aoh-change-percent-by-trend) The distribution of changes in area of habitat (AOH) as a percent of site area, shown according to trend direction. Changes in AOH are calculated using the mean AOH during the first and last 5 years of the time series.
...
(ref:caption-aoh-obs-v-pot-by-site-prop) The proportion of species with various trends in AOH at each site comparing observed abandonment (Calculation 1) and our scenario of potential abandonment without recultivation (Calculation 1P). Site proportions are shown on the left and overall totals are shown on the right, with birds on the top half and mammals on the bottom half. Counts at each site are shown in Fig. \@ref(fig:aoh-obs-v-pot-by-site). Differences in the magnitude of AOH gained or lost are shown in Figs. \@ref(fig:aoh-p-change-obs-v-pot-bird) and \@ref(fig:aoh-p-change-obs-v-pot-mam).

```{r aoh-obs-v-pot-by-site-prop, fig.cap = '(ref:caption-aoh-obs-v-pot-by-site-prop)', fig.pos = "!hbt"}
knitr::include_graphics(path = paste0(p_plots, "aoh/aoh_obs_v_pot_by_site_prop_combo",
                                      aoh_run_date,".pdf"))
...
```{r traits-mod-table-main, echo = FALSE, message = FALSE, eval = FALSE, cache = TRUE}
...
Modelled trait associations are not markedly different when assuming no recultivation took place ("potential abandonment" scenario).
...
These sites, ranging from `r round(min(area_summary_df$total_site_area_ha_2017/10^6), digits = 2)` million to `r round(max(area_summary_df$total_site_area_ha_2017/10^6), digits = 2)` million ha (Mha) in area, were selected to represent areas with documented histories of cropland abandonment and cover a range of biomes across 4 continents, including drylands, temperate regions, and the sub-tropic and wet tropics (Fig. \@ref(fig:global-map)a).
...
## Assigning land cover to habitat types {#si-habitat-assignment .unnumbered}
...
### Allocating savanna and shrubland habitats to land cover classes {#si-savanna-shrubland-jac.unnumbered}
...
## Results of statistical tests comparing proportion of winners and losers by species group {#si-prop-test-results .unnumbered}
...
# Supplemental Tables
(ref:caption-aoh-stats-table-all-obs) Number of bird and mammals with various trends in area of habitat (AOH) across different spatial and temporal calculations of AOH, when accounting for each species' trend at every site.

```{r aoh-stats-table-all-obs, tab.cap = '(ref:caption-aoh-stats-table-all-obs)', echo = FALSE, message = FALSE, eval = TRUE}
# table for MS:
# A table showing the number of species with the percent of overall in parentheses
...
```{r traits-mod-table-binary, tab.cap = '(ref:caption-traits-mod-table-binary)', echo = FALSE, message = FALSE, eval = TRUE, cache = TRUE}
...
```{r traits-mod-table-magnitude, tab.cap = '(ref:caption-traits-mod-table-magnitude)', echo = FALSE, message = FALSE, eval = TRUE, cache = TRUE}
...
```{r traits-mod-table-slope, tab.cap = '(ref:caption-traits-mod-table-slope)', echo = FALSE, message = FALSE, eval = TRUE, cache = TRUE}
trait_mod_df %>% 
  filter(aoh_type == "crop_abn_iucn", 
...
```{r traits-mod3-table, tab.cap = '(ref:caption-traits-mod3-table)', echo = FALSE, message = FALSE, eval = TRUE, cache = TRUE}
