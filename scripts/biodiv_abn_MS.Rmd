...
aoh_ms_tmp_trends_by_sp %>%
    ...
    summarise(n_sp = n()) %>% ungroup() %>%
    arrange(vert_class, aoh_type) %>% mutate(overall_trend = gsub(" ", "_", overall_trend)) %>%
  group_by(vert_class) %>% mutate(n = sum(n_sp)) %>% ungroup() %>%
  mutate(percent = 100*n_sp/n, combo = paste0(n_sp, " (", round(percent, digits = 2), "%)")) %>% 

    pivot_wider(id_cols = c(vert_class), names_from = contains("trend"), values_from = n_sp)# %>%
    pivot_wider(id_cols = c(vert_class), names_from = contains("trend"), values_from = combo)# %>%
    mutate(prop_wl = gain/loss, p = (gain + weak_gain) / (loss + weak_loss))
```
...
knitr::include_graphics(path = paste0(p_plots, "aoh/effect_size/hist_aoh_change_as_prop_site_area_obs_by_trend_5.pdf"))
```
...
```{r traits-mod-table-binary, echo = FALSE, message = FALSE, eval = FALSE, cache = TRUE}
...
```{r traits-mod-table-magnitude, echo = FALSE, message = FALSE, eval = FALSE, cache = TRUE}
...
```{r traits-mod-table-slope, echo = FALSE, message = FALSE, eval = FALSE, cache = TRUE}
trait_mod_df %>% 
  filter(aoh_type == "crop_abn_iucn", 
...
```{r traits-mod3-table, echo = FALSE, message = FALSE, eval = FALSE, cache = TRUE}
...
(ref:caption-log-ratio-aoh-change-by-site) The distribution of the natural logarithm of the relative change in AOH ("ratio") of final AOH to starting AOH. Changes in AOH are calculated using the mean AOH during the first and last 5 years of the time series.

```{r log-ratio-aoh-change-by-site, fig.cap = '(ref:caption-log-ratio-aoh-change-by-site)'}
knitr::include_graphics(path = paste0(p_plots, "aoh/effect_size/hist_aoh_log_ratio_obs_5.pdf"))
```

(ref:caption-ratio-aoh-change-by-site) The distribution of the natural logarithm of the relative change in AOH ("ratio") of final AOH to starting AOH. Changes in AOH are calculated using the mean AOH during the first and last 5 years of the time series.
