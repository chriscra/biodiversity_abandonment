...
# ------------------------------------ #
# do threatened species really have a statistically significant higher proportion of winners?
...
p_test_iucn_all <-
  prop.test(x = aoh_gain_thr_v_non_thr_tmp %>% 
              group_by(threatened) %>% 
              summarise(n_sp = sum(n_sp), N = sum(N)) %>%
              .$n_sp,
            n = aoh_gain_thr_v_non_thr_tmp %>% 
              group_by(threatened) %>% 
              summarise(n_sp = sum(n_sp), N = sum(N)) %>%
              .$N)
...
# birds
p_test_iucn_bird <-
  prop.test(x = aoh_gain_thr_v_non_thr_tmp %>% filter(vert_class == "bird") %>% .$n_sp,
            n = aoh_gain_thr_v_non_thr_tmp %>% filter(vert_class == "bird") %>% .$N)

# mammals
p_test_iucn_mam <-
  prop.test(x = aoh_gain_thr_v_non_thr_tmp %>% filter(vert_class == "mam") %>% .$n_sp,
            n = aoh_gain_thr_v_non_thr_tmp %>% filter(vert_class == "mam") %>% .$N)

# now, including weak trends:
aoh_gain_thr_v_non_thr_incl_weak_tmp <-
  aoh_trends_by_sp %>%
  filter(passage_type == "exclude_passage",
         vert_class != "amp", 
         !aoh_type %in% c("abn_iucn", "potential_abn_iucn"),
...
p_test_iucn_bird_incl_weak <-
  prop.test(x = aoh_gain_thr_v_non_thr_incl_weak_tmp %>% filter(vert_class == "bird") %>% .$n_sp,
            n = aoh_gain_thr_v_non_thr_incl_weak_tmp %>% filter(vert_class == "bird") %>% .$N)

# mammals
p_test_iucn_mam_incl_weak <-
  prop.test(x = aoh_gain_thr_v_non_thr_incl_weak_tmp %>% filter(vert_class == "mam") %>% .$n_sp,
            n = aoh_gain_thr_v_non_thr_incl_weak_tmp %>% filter(vert_class == "mam") %>% .$N)
...
p_test_m_v_b_incl_weak <- 
  prop.test(x = aoh_gain_bird_v_mam_incl_weak_tmp$n_sp,
          n = aoh_gain_bird_v_mam_incl_weak_tmp$N)

```


Mammals had a statistically significantly greater proportion of consistent winners than birds [$X^2$ (df = `r p_test_m_v_b$parameter`, *N* = `r sum(aoh_gain_bird_v_mam_tmp$N)`) = `r p_test_m_v_b$statistic`, *p* `r ifelse(p_test_m_v_b$p.value < 0.01,"< 0.01", round(p_test_m_v_b$p.value, digits = 2))`].
...
If former fields recultivated within a decade or two, ecosystems may not have enough time to accumulate substantial amounts of carbon or biodiversity.
...
(i) Did cropland abandonment result in gains or losses of habitat for individual bird and mammal species, and how does the balance of winners and losers vary across sites and by rarity of species (IUCN threat status)?
...
For our analysis we used a 30-m annual land cover time series produced for eleven sites from 1987-2017 [@Yin2020] (Figs. \@ref(fig:lc-1987) and \@ref(fig:lc-2017)).
These sites, ranging from `r round(min(area_summary_df$total_site_area_ha_2017/10^6), digits = 2)` million to `r round(max(area_summary_df$total_site_area_ha_2017/10^6), digits = 2)` million ha (Mha) in area, were selected to represent areas with documented histories of cropland abandonment and cover a range of biomes across four continents, including drylands, temperate regions, and the sub-tropic and wet tropics (Fig. \@ref(fig:global-map)).
...
We excluded marine species and species classified as "Extinct" (EX) or "Extinct in the Wild" (EW), and only considered areas where species were considered “extant” and either "native" or "reintroduced," leaving 1495 bird and 528 mammal species (2023 total) with ranges overlapping at least one of our eleven sites.
...
First, we developed a simple crosswalk between the coarse land cover types (e.g., forest) delineated in @Yin2020 and the more specific habitat types considered by the IUCN (e.g., temperate forest), based on how each habitat type would be categorized during classification of satellite imagery into land cover (see Table \@ref(tab:iucn-crosswalk-table)).
...
(ref:caption-aoh-method-panel) Three spatial and temporal scopes for our AOH calculations.  Rows represent Cases 1, 2, and 3 (top to bottom), showing the scales across which we calculated AOH, ranging abandonment only (Case 1), to the entire site extent for the full time series (Case 3). The leftmost column shows a land cover trajectory for a single pixel to illustrate the temporal scope of our AOH calculation (i.e., when a given pixel is considered in each Case) the sake of illustration, and the dashed line represents the year this pixel was first cultivated. The middle column shows the spatial scope for which AOH was calculated, either abandoned pixels only (Case 1 and Case 2) or the entire site extent (Case 3), using Bosnia & Herzegonia as an example. The rightmost column shows AOH (in Mha) for each Case for one example species, *Turdus iliacus* (redwing), at Bosnia & Herzegovina, along with linear trend and 95% confidence intervals.

```{r aoh-method-panel, fig.cap = '(ref:caption-aoh-method-panel)', fig.pos = "!ht"}
...
To isolate the effect of cropland abandonment on habitat availability, we first considered just changes in area of habitat stemming directly from cropland abandonment ("Case 1").
In order to discern the effect of cropland abandonment *per se*, we calculated the area of habitat provided by pixels that were abandoned at some point during the time series. 
We excluded habitat provided by these pixels prior to cultivation, therefore considering only habitat provided by these pixels when classified as cropland, abandoned, or, where appropriate, recultivated (see Fig. \@ref(fig:aoh-methods-panel)).
This step allowed us to explicitly consider the habitat value provided by abandoned croplands before and after abandonment, therefore isolating the effect of this specific land use change.

Then, in order to put the AOH changes resulting directly from abandonment into a broader context of habitat loss and land use changes throughout each site, we calculated AOH for two additional cases with wider temporal and spatial scopes.
Our second case ("Case 2") considered changes in habitat that took place exclusively in pixels that experienced abandonment at some point during the time series (following Case 1), but now measured for the full time series, from 1987 through 2017.
By including the habitat provided by abandoned pixels before they were first cultivated, we capture the effect of habitat changes that occurred before cultivation and subsequent abandonment (e.g., deforestation to generate the cropland that was subsequently abandoned).
This allows us to consider the habitat contributions of abandonment relative to a simple baseline set to 1987 (the start of our time series).

Finally, we calculated AOH for each species across the entire landscape at each site ("Case 3"), for across the full time series (1987-2017), capturing the overall trend in AOH that each species experienced during our study period.
Therefore, this case placed abandonment into the context of broader land cover change dynamics at each site and allowed us to consider the extent to which abandonment compensated for land cover changes taking place outside of abandonment.
...
These potential abandonment scenarios follow the same spatial and temporal constraints as the cases described above and are designated as cases "1P," "2P," and "3P."
...
Overall, we found that a majority of bird and mammal species gained habitat as a result of abandonment: `r aoh_stats_main$n_gain_bird` bird and `r aoh_stats_main$n_gain_mam` mammal species showed statistically significant and positive trends in AOH over time everywhere they occurred, compared to `r aoh_stats_main$n_loss_bird` bird and `r aoh_stats_main$n_loss_mam` mammal species that showed statistically significant losses in habitat everywhere they occurred (Fig. \@ref(fig:aoh-by-site-crop-abn)).
An additional `r aoh_stats_main$n_w_gain_bird` birds and `r aoh_stats_main$n_w_gain_mam` mammals had “weak” gains in AOH, while `r aoh_stats_main$n_w_loss_bird` birds and `r aoh_stats_main$n_w_loss_mam` mammals showed “weak” losses in AOH (i.e., significant gains or losses, respectively, at some sites, but no significant trend at other sites).
`r aoh_stats_main$n_opposite_bird` birds and `r aoh_stats_main$n_opposite_bird` mammals showed significant gains at some sites, and significant losses elsewhere (“Opposites”). 
...
When including “weak” winners and losers, the ratio of winners to losers were `r aoh_stats_main$wl_prop_bird_sp_w_weak` and `r aoh_stats_main$wl_prop_mam_sp_w_weak` for birds and mammals respectively.
...
When considering only threatened species, we found that a greater proportion of threatened species gained habitat as a result of abandonment than lost habitat (Fig. \@ref(fig:aoh-iucn-by-site-crop-abn)), though there were relatively few threatened species per site (Fig. \@ref(fig:richness-by-site)) and the proportions of winners were not statistically significantly different for threatened and non-threatened species [two-sample proportion test with continuity correction yielded: $X^2$ (df = `r p_test_iucn_all$parameter`, *N* = `r aoh_gain_thr_v_non_thr_tmp %>% .$N %>% sum()`) = `r p_test_iucn_all$statistic`, *p* `r ifelse(p_test_iucn_all$p.value < 0.01,"< 0.01", round(p_test_iucn_all$p.value, digits = 2))`].
...
## Comparing proportion of winners and losers by species group

We used two-sample Z-tests to test whether threatened and non-threatened species had equal proportions of winners, with Yates' continuity correction.
We found that the difference between these two proportions was not statistically significant [$X^2$ (df = `r p_test_iucn_all$parameter`, *N* = `r aoh_gain_thr_v_non_thr_tmp %>% .$N %>% sum()`) = `r p_test_iucn_all$statistic`, *p* `r ifelse(p_test_iucn_all$p.value < 0.01,"< 0.01", round(p_test_iucn_all$p.value, digits = 2))`].
This was also the case for birds [$X^2$ (df = `r p_test_iucn_bird$parameter`, *N* = `r aoh_gain_thr_v_non_thr_tmp %>% filter(vert_class == "bird") %>% .$N %>% sum()`) = `r p_test_iucn_bird$statistic`, *p* `r ifelse(p_test_iucn_bird$p.value < 0.01,"< 0.01", round(p_test_iucn_bird$p.value, digits = 2))`] and mammals [$X^2$ (df = `r p_test_iucn_mam$parameter`, *N* = `r aoh_gain_thr_v_non_thr_tmp %>% filter(vert_class == "mam") %>% .$N %>% sum()`) = `r p_test_iucn_mam$statistic`, *p* `r ifelse(p_test_iucn_mam$p.value < 0.01,"< 0.01", round(p_test_iucn_mam$p.value, digits = 2))`] individually.
