...
(Statement about effect sizes?
Statement about traits?)
...
Although habitat loss from the conversion of natural habitats to agriculture remains the most widespread threat to terrestrial biodiversity [@Tilman2017], with agricultural expansion continuing to threaten many places [@Potapov2022], agricultural land use change is not unidirectional.
...
As agriculture grows more efficient and rural outmigration, urbanization, and globalization continue, some expect abandonment to intensify in the future [@Sanderson2018; @Taylor2021], and environmentalists increasingly look to former agricultural lands for low-cost opportunities to restore ecosystems [@Navarro2012], sequester carbon [@Griscom2017; @Yang2020], and recover biodiversity [@Chazdon2020; @Xie2019; @Baumann2020].

Importantly, however, biodiversity is not guaranteed to benefit when cropland is abandoned.
At a fundamental level, given that individual species have unique biophysical and habitat requirements and are not evenly distributed around the world, spatial and temporal variation in abandonment will substantially affect biodiversity outcomes.
More practically, the recovery of natural ecosystems requires time, multiple decades in many ecosystems [@Acevedo-Charry2019; @Isbell2019; @Nerlekar2020], and if former fields are recultivated within a decade or two, ecosystems may not have enough time to accumulate substantial amounts of carbon or biodiversity.
...
When considering what could be lost, abandonment is often considered a threat to biodiversity when it reduces landscape heterogeneity and the amount of open habitats, regardless of the potential for gains in other types of species [@Queiroz2014; @Navarro2012].
Given that species have unique responses to land use change [@Sirami2008; @Finch2019], conservationists need a more nuanced understanding of which species benefit, which are harmed, and how these patterns balance out across whole landscapes in order to meaningfully improve how biodiversity is incorporated into policy responses to abandonment.

Though many studies have investigated the impact of abandonment on local biodiversity over time [e.g., @Bowen2007; @Sirami2008; @Uchida2014; @Plieninger2014; @Regos2016; @Isbell2019], few have attempted to investigate how abandonment affects biodiversity at the landscape scale by combining maps of abandonment with spatially-explicit biodiversity data.
Crucially, abandonment maps used in past analyses have generally been limited in spatial and temporal resolution, lacking the detail required to track abandonment dynamics through time.
...
These new time series data have allowed for more detailed analyses of the long-term persistence of abandoned cropland [@Crawford2022], and they also better equip researchers to investigate how abandonment affects individual species across multiple landscapes and longer time horizons.
...
aoh_table_tmp1 %>%
  ...
  {if (doc_type == "word") 
    flextable(.) %>% 
      ...
      set_caption(., caption = '(ref:caption-main-aoh-stats-table)',
                  html_escape = TRUE) %>%
      autofit(.)
...
Overall, we found that a majority of bird (`r aoh_stats_main$n_gain_bird_percent`%) and mammal species (`r aoh_stats_main$n_gain_mam_percent`%) at our sites consistently gained habitat as a result of abandonment, experiencing statistically significant and positive trends in AOH everywhere they occurred (Table \@ref(tab:main-aoh-stats-table), Fig. \@ref(fig:aoh-by-site-crop-abn)).
In contrast, only `r aoh_stats_main$n_loss_bird_percent`% of bird and `r aoh_stats_main$n_loss_mam_percent`% of mammal species experienced significant losses of habitat consistently across sites (Table \@ref(tab:main-aoh-stats-table), Fig. \@ref(fig:aoh-by-site-crop-abn)).
Some species exhibited contrasting trends at different sites, accounting for an additional `r aoh_stats_main$n_w_gain_bird_percent`% of birds and `r aoh_stats_main$n_w_gain_mam_percent`% of mammals with a combination of increasing or insignificant trends at different sites (i.e., “weak” gains), and `r aoh_stats_main$n_w_loss_bird_percent`% and `r aoh_stats_main$n_w_loss_mam_percent`% respectively had a combination of decreasing or insignificant trends (i.e., “weak” losses; Table \@ref(tab:main-aoh-stats-table)).
...
The lone exception were birds in Iraq (our site with the lowest species richness; Fig. \@ref(fig:richness-by-site)), where `r 1-aoh_stats_main$wl_iraq`% more birds lost habitat than gained (Figs. \@ref(fig:aoh-by-site-crop-abn-prop) and \@ref(fig:aoh-by-site-crop-abn)).
In general, a relatively greater share of mammal species gained habitat than birds species, both at individual sites (Fig. \@ref(fig:aoh-by-site-stacked)) and overall (Fig. \@ref(fig:aoh-overall-stacked)) [two-sample proportion test: $X^2$ (df = `r p_test_gain_m_v_b$parameter`, *N* = `r sum(aoh_gain_bird_v_mam_tmp$N)`) = `r round(p_test_gain_m_v_b$statistic, digits = 2)`, *p* `r ifelse(p_test_gain_m_v_b$p.value < 0.01,"< 0.01", paste0("= ", round(p_test_gain_m_v_b$p.value, digits = 2)))`].
Though few threatened species occurred at our sites (Fig. \@ref(fig:richness-by-site)), a greater proportion gained habitat than lost habitat following abandonment (Fig. \@ref(fig:aoh-iucn-by-site-crop-abn)).
However, this proportion was not significantly different from that for non-threatened species [two-sample proportion test: $X^2$ (df = `r p_test_iucn_all$parameter`, *N* = `r aoh_gain_by_iucn_tmp %>% .$N %>% sum()`) = `r prettyNum(round(p_test_iucn_all$statistic, digits = 2), nsmall = 2)`, *p* `r ifelse(p_test_iucn_all$p.value < 0.01,"< 0.01", paste0("= ", round(p_test_iucn_all$p.value, digits = 2)))`].
Even fewer small-ranged birds and mammals occurred at our sites (3 birds and 6 mammals), precluding statistical significance, yet nearly all gained habitat as a result of abandonment (Fig. \@ref(fig:aoh-by-range)).

```{r effect-size-stats, include = FALSE, eval = TRUE}
...
mean_effect_size_df %>%
  mutate(aoh_type = case_when(str_detect(aoh_type, "potential") ~ "pot",
                              TRUE ~ "obs")) %>%
    pivot_wider(id_cols = c(aoh_type, stat), 
                names_from = trend, values_from = value)
...
```{r traits-mod-table-main, echo = FALSE, message = FALSE, eval = TRUE, cache = TRUE}
trait_mod_df <- readRDS(file = paste0(p_derived, "traits/", "trait_mod_df_mod1.rds"))
  ...
  {if (doc_type == "word") 
    as_flex_table(.) %>%
      ...
      set_caption(., 
                  ...
                  html_escape = TRUE) %>%
      autofit(.) else as_kable_extra(., caption = '(ref:caption-traits-mod-table-main)')}
```
...
Whether arable land is suitable habitat for a species is the strongest predictor of whether a species will gain or lose habitat following cropland abandonment, which is no surprise. When excluding arable land and urban land suitability as predictors in our models, a range of other predictors become statistically significant, including habitat breadth, vertebrate class, and trophic level. For example, when excluding arable land suitability as a predictor in the binary models, savanna drops out, as does wetlands, and habitat breadth, vertebrate class, trophic level, range area, tropicality, and abandonment extent now start playing a role.
...
While it is possible that this represents a true relationship, meaning that more generalist species are more likely to lose habitat as a result of abandonment.
However, it may be more likely that this change reflects the fact that  


Modeled trait associations are not markedly different when assuming no recultivation took place ("potential abandonment" scenario).

## Effects of recultivation on habitat area {.unnumbered}

Many abandoned croplands were recultivated during our time series [38.05% over average across sites, @Crawford2022], with substantial impacts on species’ AOH. Had no recultivation taken place during our time series, the percentage of consistent winners would have increased `r paste0("from ", aoh_stats_main$n_gain_bird_percent, "% to ", obs_v_pot_stats$n_gain_bird_pot_percent, "% for birds and ", aoh_stats_main$n_gain_mam_percent, "% to ", obs_v_pot_stats$n_gain_mam_pot_percent, "% for mammals")` (Table \@ref(tab:main-aoh-stats-table), Fig. \@ref(fig:aoh-obs-v-pot-by-site)), both statistically significant increases [two-sample proportion tests: birds - $X^2$ (df = `r obs_v_pot_stats$bird_gain_p_test$parameter`, *N* = `r aoh_overall_obs_v_pot_tmp %>% filter(vert_class == "bird", overall_trend == "gain") %>% .$N %>% sum()`) = `r round(obs_v_pot_stats$bird_gain_p_test$statistic, digits = 2)`, *p* `r ifelse(obs_v_pot_stats$bird_gain_p_test$p.value < 0.01,"< 0.01", paste0("= ", round(obs_v_pot_stats$bird_gain_p_test$p.value, digits = 2)))`; mammals - $X^2$ (df = `r obs_v_pot_stats$mam_gain_p_test$parameter`, *N* = `r aoh_overall_obs_v_pot_tmp %>% filter(vert_class == "mam", overall_trend == "gain") %>% .$N %>% sum()`) = `r round(obs_v_pot_stats$mam_gain_p_test$statistic, digits = 2)`, *p* `r ifelse(obs_v_pot_stats$mam_gain_p_test$p.value < 0.01, "< 0.01", paste0("= ", round(obs_v_pot_stats$mam_gain_p_test$p.value, digits = 2)))`].
...
Though abandonment *per se* benefitted many more species than it harmed (Table \@ref(tab:main-aoh-stats-table)), did it compensate for 1) habitat that was cleared for croplands prior to abandonment or 2) habitat loss that occurred throughout through the entire site?
When considering the net AOH change in lands that experienced abandonment or throughout the entire landscape at each site, the balance of winners to losers becomes much less favorable (Fig. \@ref(fig:aoh-overall)).
In both of these broader AOH calculations, more bird species lost habitat than gained, and about equal numbers of mammals gained as lost, maintaining a marginally positive ratio of winners to losers (Table \@ref(tab:main-aoh-stats-table), Figs. \@ref(fig:aoh-overall-stacked-w-pot) and \@ref(fig:aoh-overall-w-pot).
Both of these declines in the proportion of winning species were statistically significant (see SI Section \@ref(si-prop-test-results)).

While the number of losing species increased at all sites (Fig. \@ref(fig:aoh-by-site-stacked) when factoring in broader land cover changes these increases were most severe in Mato Grosso, Brazil, where relatively less abandonment and relatively more land cover change occurred than at other sites, affecting a large number of tropical forest species (see site-specific results in Figs. \@ref(fig:aoh-by-site-max-abn) and \@ref(fig:aoh-by-site-full)).
Our sites in Bosnia & Herzegovnia; Chongqing, China; Goiás, Brazil; and Iraq also showed negative ratios of winners to losers, but sites elsewhere managed to narrowly maintain positive ratios (particularly for mammal species), despite these ratios declining on the whole.
...
Though we only had three sites in the tropics (precluding any pantropical conclusions; Figs. \@ref(fig:global-map-w-wisc)a and \@ref(fig:site-biomes)), these sites showed the highest number of winner species, led by Mato Grosso, Brazil and Goiás, Brazil.
...
In our primary model, habitat breadth had a weakly positive association with significant gains in habitat, which may be because greater habitat breadth also means higher likelihood of arable land suitability.
However, when 
...
To account for differences in the overall magnitude of abandonment at each site, we also included the percentage of each site that was abandoned during the time series, derived from -@Crawford2022.
...
We next investigate how various species traits (e.g., habitat suitability, habitat specialization, threat status, body mass, range size, trophic level, etc.) correlate with changes in AOH that took place following cropland abandonment. We found that
...
```{r aoh-stats-table-all-obs, echo = FALSE, message = FALSE}
# table for MS:
# A table showing the number of species with the percent of overall in parentheses
...
```{r aoh-stats-by-site, echo = FALSE, message = FALSE}
# ----------------------------------------------------------------- #
...
#   mutate(trend = gsub(" ", "_", trend)) %>%
#   group_by(aoh_type, site, vert_class) %>% mutate(n = sum(n_sp)) %>% ungroup() %>%
#   mutate(percent = 100*n_sp/n, combo = paste0(n_sp, " (", round(percent, digits = 2), "%)")) 
# 
...
# print Table
aoh_ms_tmp_trends %>%
    group_by(site, aoh_type, vert_class,
             trend) %>%
    summarise(n_sp = n()) %>% ungroup() %>%
    arrange(site, aoh_type, vert_class
            ) %>% 
  mutate(trend = gsub(" ", "_", trend)) %>%
  ...
  colformat_double(., digits = 2) %>%
  fontsize(., size = 10, part = "all") %>%
  set_caption(., caption = "Number of bird and mammal species with statistically significant gains or losses in AOH following cropland abandonment at each site.", html_escape = TRUE) %>%
  autofit(.)
```

```{r aoh-changes-obs-v-pot, echo = FALSE, message = FALSE}
...
n_aoh_trend_changes_by_site_df %>% 
  ...
  # colformat_double(., digits = 2) %>%
  fontsize(., size = 10, part = "all") %>%
  set_caption(., caption = "Number of additional species with statistically significant increasing or decreasing trends in AOH for a scenario in which no abandoned croplands were recultivated.", html_escape = TRUE) %>%
  autofit(.)
...
iucn_crosswalk %>%
  ...
  {if (doc_type == "word") 
    flextable(.) %>% 
      ...
      set_caption(., caption = '(ref:caption-iucn-crosswalk-table)',
                  html_escape = TRUE) %>%
      fontsize(., size = 10, part = "all") %>%
      autofit(.)
    else kbl(., 
...
trait_mod_df %>% 
  ...
  {if (doc_type == "word") 
    as_flex_table(.) %>% 
    theme_booktabs(.) %>%
    set_caption(caption = '(ref:caption-traits-mod-table-binary)',
                html_escape = TRUE) %>%
      fontsize(., size = 10, part = "all") %>%
      autofit(.)
  else as_kable_extra(., caption = '(ref:caption-traits-mod-table-binary)')}
```

(ref:caption-traits-mod-table-magnitude) Trait model coefficients for binary trend response variables.

```{r traits-mod-table-magnitude, echo = FALSE, message = FALSE, eval = TRUE}
...
trait_mod_df %>% 
  ...
  {if (doc_type == "word") 
    as_flex_table(.) %>%
      ...
      fontsize(., size = 10, part = "all") %>%
      autofit(.)
    else as_kable_extra(., caption = '(ref:caption-traits-mod-table-magnitude)')}
```
...
trait_mod_df %>% 
  ...
  {if (doc_type == "word") 
    as_flex_table(.) %>%
      ...
      fontsize(., size = 10, part = "all") %>%
      autofit(.)
    else as_kable_extra(., caption = '(ref:caption-traits-mod-table-slope)')}
...
```{r traits-mod3-table, echo = FALSE, message = FALSE, eval = TRUE, cache = TRUE}
readRDS(file = paste0(p_derived, "traits/", "trait_mod_df_mod3.rds")) %>%
  ...
  {if (doc_type == "word") 
    as_flex_table(.) %>%
      ...
      fontsize(., size = 10, part = "all") %>%
      autofit(.)
    else as_kable_extra(., caption = '(ref:caption-traits-mod3-table)')}
```
...
```{r aoh-obs-v-pot-by-site-prop, fig.cap = '(ref:caption-aoh-obs-v-pot-by-site-prop)', fig.pos = "!hbt"}
...
knitr::include_graphics(path = paste0(p_plots, "aoh/traits/model_diagnostics_resid_v_fitted_combo.pdf"))
```
...
knitr::include_graphics(path = paste0(p_plots, "aoh/traits/model_diagnostics_qq_combo.pdf"))
```
