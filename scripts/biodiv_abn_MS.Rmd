---
title: "Cropland abandonment provides habitat for a plurality of birds and mammals, but rarely compensates for habitat loss"
...
In order to calculate AOH, we had to translate our land cover maps into maps of habitats to match the habitat suitabilities listed by IUCN. 
To do so, we used a recently published 100-m global map of IUCN habitat types for the year 2015 developed by @Jung2020a.
We developed a simple crosswalk between our coarse land cover types (woody vegetation, herbaceous vegetation, non-vegetation, and cropland) and the more specific habitat types considered by the IUCN (e.g., temperate forest) (see Table \@ref(tab:iucn-crosswalk-table)).
...
-	Translating the land cover maps into habitat maps, using focal filters (modal)
 


In order to translate this map of potential abandonment into potential land cover (and subsequently potential habitat types), we...

<!-- uncovered that substantial abandonment occurred at these sites (8.76 Mha of croplands were abandoned at least one time from 1987 through 2017), but that this abandonment was relatively short-lived, and subject to high rates of recultivation (only 6.06 Mha of croplands remained abandoned as of 2017).  -->
...
We extracted only species classified as "extant," excluded species marine mammals, species assessed as "Extinct" or "Extinct in the Wild,"
...
### AOH types
We calculated AOH across three geographic and temporal scopes.
In order to isolate the effect of cropland abandonment explicitly on AOH, we first considered only changes to habitat area for only those pixels that were abandoned at some point during the time series.
Furthermore, we considered habitat beginning only when a given pixel was first cultivated through the end of the time series (i.e., "from first cultivation through abandonment").

Next, we calculated AOH for all pixels that were abandoned at least once during the time series, but for all years (1987-2017).
This effectively captured any habitat changes that occurred preceding cultivation and subsequent abandonment (e.g., deforestation to allow for cultivation).

Finally, we calculated AOH for the full landscape at each site, across the full time series, therefore capturing the overall changes in AOH that each species experienced, including abandonment but also other land cover change dynamics. 

We also calculated AOH for a scenario of potential abandonment, in which no recultivation took place following abandonment (i.e., all abandoned pixels remained abandoned from the year of initial abandonment through the end of the time series, following @Crawford2022).
...
However, the single exception was Mato Grosso, Brazil, where 70 additional bird species showed statistically significant positive trends in AOH during our time series (231 compared to 161), largely resulting from the transition of many species from "no trend" to "gain" species.
...
(ref:caption-aoh-by-site-crop-abn) Number of species by trend in AOH over time.
...
(ref:caption-aoh-trend-lines-crop-abn) AOH trends for all species occurring at each site, for only abandoned pixels, from cultivation through 2017.
...
- Fragmentation
- Habitat preferences as a function of age of habitat, succession. 
...
knitr::include_graphics(path = paste0(p_plots, "aoh/trend_summary_by_site_full_iucn",
                                      aoh_run_date,".pdf"))
```
...
knitr::include_graphics(path = paste0(p_plots, "aoh/trend_summary_by_site_max_abn_iucn",
                                      aoh_run_date,".pdf"))
```
...
(ref:caption-aoh-obs-v-pot-by-site) AOH for observed vs. potential abandonment, by site and AOH scale.
...
1. Table with crosswalk of land cover to habitat types
2. (TBD) Mapping the habitat for one species in particular, at one site, with the habitat (maybe future)
