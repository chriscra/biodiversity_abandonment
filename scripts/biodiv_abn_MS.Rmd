...
(ref:caption-aoh-change-proportion-by-site) The distribution of changes in area of habitat (AOH) as a proportion of site area, shown according to directional trend.

```{r aoh-change-proportion-by-site, fig.cap = '(ref:caption-aoh-change-proportion-by-site)'}
...
```{r traits-mod-binary-table, echo = FALSE, message = FALSE, eval = TRUE}
test_gt <- 
  glm_mod_df %>% 
  filter(aoh_type == "crop_abn_iucn"#, str_detect(response, "gain")
         ) %>% 
  mutate(gt = lapply(mod, function(model) {
         model %>%
           tbl_regression(
...
(ref:caption-dw-stat) Durbin-Watson statistics for trends of AOH for Calculation 1.
...
(ref:caption-aoh-change-proportion-by-site) The distribution of changes in area of habitat (AOH) as a proportion of site area at each site.

```{r aoh-change-proportion-by-site, fig.cap = '(ref:caption-aoh-change-proportion-by-site)'}
knitr::include_graphics(path = paste0(p_plots, "aoh/effect_size/hist_aoh_change_as_prop_site_area_obs_by_site_5.pdf"))
```
