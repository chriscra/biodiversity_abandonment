...
However, a greater share of species would have benefitted overall (`r abstract_stats$percent_winner_bird_pot`% and `r abstract_stats$percent_winner_mam_pot`% respectively) if no abandoned croplands had been recultivated.
...
Yet despite substantial evidence of recent abandonment, associated biodiversity benefits are not guaranteed.
...
If abandonment is relatively short-lived and former fields are recleared and recultivated within a decade or two, as recent evidence indicates is common [@Crawford2022; @Dara2018; @Reid2018b; @Schwartz2020], ecosystems may not have enough time to accumulate substantial amounts of carbon or biodiversity.
Given that species are not equally distributed around the world, and that individual species have unique biophysical and habitat requirements, spatial and temporal variation in abandonment may also play a substantial role in determining outcomes for biodiversity.
In addition, ecologists disagree about how to maximize biodiversity within agricultural landscapes, whether by pairing a relatively small footprint of high-intensity agriculture with intact natural habitats ["land sparing," e.g., @Phalan2011] or by cultivating a relatively larger area of lower intensity agriculture to maintain high habitat heterogeneity within the landscape ["land sharing," e.g., @Kremen2018].
...
Moreover, these few studies have done so only coarsely, whether using distance to protected areas as a proxy for biodiversity value [@Meyfroidt2016], expert-designated regional conservation priorities such as "biodiversity hotspots" [@Naess2021], or a restricted subset of species [@VanderZanden2017].
<!-- Moreover, these few studies have done so only coarsely, whether using distance to protected areas as a proxy for biodiversity value [@Meyfroidt2016], expert-designated regional priorities such as "biodiversity hotspots" [@Naess2021; @Henry2019], coarse measures of species richness [@Henry2019], or a restricted subset of species [@VanderZanden2017]. -->
<!-- Some studies have used individual species data to prioritize locations for restoration based on estimated reductions in extinction risk [e.g., @Strassburg2019; @Crouzeilles2020], but these approaches have not been linked to observed changes in land cover through time nor explored the nuances of which species benefit. -->
Most critically, the abandonment maps used in these analyses have historically been limited in spatial and temporal resolution, lacking the detail required to track abandonment dynamics through time.

Recent efforts to improve how abandonment is mapped from satellite imagery have produced annual maps of cropland abandonment with higher spatial resolution (30-m resolution) and temporal coverage (1987-2017) than previous maps [@Yin2020]. 
These new time series data have allowed for more detailed analysis of the long-term trajectories and persistence of abandoned cropland [@Crawford2022], and also better equip researchers to investigate the implications of abandonment for individual species across multiple landscapes and longer time horizons.
...
(ii) How did recultivation affect the overall habitat implications of abandonment, when comparing observed abandonment and recultivation to a scenario in which recultivation was not allowed to occur? 
(iii) Finally, how did the changes resulting directly from abandonment compare to broader habitat changes that occurred a) prior to abandonment, or b) outside of abandonment (i.e., throughout the entire landscape) at each site?
...
Together, these results highlight opportunities to improve the biodiversity outcomes of abandonment, and better harness broad agricultural land-use transitions for conservation.
...
These sites, each between `r round(min(area_summary_df$total_site_area_ha_2017/10^6), digits = 2)` and `r round(max(area_summary_df$total_site_area_ha_2017/10^6), digits = 2)` million ha (Mha) in area^[For context, the state of New Jersey is about 1.90 Mha], were selected to represent areas with documented histories of cropland abandonment, and cover a range of biomes across four continents, including drylands, temperate regions, and the sub-tropic and wet tropics (Fig. \@ref(fig:global-map)).
...
This time series mapped four land cover classes at high accuracy (average overall accuracy of $85\pm4$%): (i) cropland, (ii) herbaceous vegetation (e.g., grassland), (iii) woody vegetation (e.g., forest), and (iv) non-vegetation (e.g., water, urban, or barren land).
...
Because AOH is calculated based on habitat, not land cover, we translated our land cover maps to match the habitat suitabilities listed by IUCN using a recently published 100-m global map of IUCN habitat types for the year 2015 [@Jung2020a], resampled to 30-m to match our land cover maps.
First, we developed a simple crosswalk between our coarse land cover types (e.g., forest) and the more specific habitat types considered by the IUCN (e.g., temperate forest), based on how each habitat type would be categorized during classification of satellite imagery into land cover (see Table \@ref(tab:iucn-crosswalk-table)).
...
Our analysis is built on the calculation of a species' Area of Habitat [AOH, *sensu* @Brooks2019], defined as the total area of a species range that falls within suitable habitat types and elevations, as derived from IUCN assessments.
...
We excluded marine species and species classified as "Extinct" (EX) or "Extinct in the Wild" (EW), and only considered areas where species were considered “extant” and either "native" or "reintroduced," leaving 1495 bird and 528 mammal species (2023 total) with ranges overlapping with at least one of our eleven sites.
...
Because species' habitat preferences listed in IUCN assessments do not directly account for habitat age, we conservatively excluded species if they required mature habitats in at least one season at each site. 
Bird species were classified as mature forest obligates if they required habitats older than 30 years (i.e., the maximum age of abandoned croplands during our time series), according to habitat descriptions listed in IUCN assessments and individual species accounts in The Handbook of the Birds of the World [@BOW2022].
...
## Calculating AOH across multiple temporal and spatial scenarios {.unnumbered}

To isolate the effect of cropland abandonment on habitat availability, we first considered only changes in habitat stemming directly from cropland abandonment ("Scenario 1") by restricting AOH to only pixels that were abandoned at some point during the time series.
In order to isolate the effect of cropland abandonment *per se*, we only considered habitat provided by abandoned pixels from the year each pixel was first cultivated through the end of the time series.
In effect, this meant only considering habitat provided by abandoned pixels when they were either under cultivation or subsequently abandoned (i.e., non-cropland).

In order to put the AOH changes resulting directly from abandonment into a broader context of habitat loss and land use changes throughout each site, we calculated AOH for two wider temporal and spatial scopes. 
Our second scenario considered changes in habitat that took place in abandoned pixels relative to the start of our time series ("Scenario 2").
Like our first scenario, we restricted AOH to only pixels that experienced abandonment, but now considered the complete trend in habitat across the full time series (1987-2017), to capture any habitat changes that occurred before cultivation and subsequent abandonment (e.g., deforestation).
Finally, we calculated AOH for the entire landscape at each site, across the entire time series (1987-2017), therefore capturing the overall trend in AOH that each species experienced during our study period, including abandonment along with other land cover change dynamics ("Scenario 3").
...
In order to assess the impact of recultivation on how abandonment affected AOH, we also calculated slight variations of these three scenarios for a scenario of "potential abandonment" in which no recultivation took place following abandonment [following @Crawford2022].
...
These potential abandonment scenarios follow the same spatial and temporal constraints as described above and are designated as "Scenario 1P," "Scenario 2P," and "Scenario 3P."
...
AOH trends were classified as either gains or losses of habitat over time when slope coefficients were positive or negative, respectively, and statistically significant ($p<0.05$, i.e., the 95% confidence interval of the slope coefficient did not overlap with zero).
...
(ref:caption-aoh-overall-stacked) Overall proportion of trend types across sites, for birds and mammals, for three AOH scenarios (1, cropland abandonment only; 2, abandonment 1987-2017; and 3, the entire landscape 1987-2017).
...
(ref:caption-aoh-overall-stacked-w-pot) Overall proportion of trend types across sites, for birds and mammals, for three AOH types (cropland abandonment only, abandonment 1987-2017, and the entire landscape 1987-2017), for both observed and potential abandonment.
