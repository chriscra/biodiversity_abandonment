...
Though agricultural expansion continues in many regions, substantial areas of cropland have been abandoned in recent decades due to demographic, socioeconomic, and technological changes.
...
While abandonment represents an important conservation opportunity, limiting recultivation and reducing habitat loss are essential if abandonment is to contribute to biodiversity conservation and recovery.
...
As agriculture intensifies and rural outmigration, urbanization, and globalization continue, abandonment is likely to accelerate [@Sanderson2018; @Taylor2021], and former agricultural lands could offer low-cost opportunities to restore ecosystems [@Navarro2012], sequester carbon [@Griscom2017; @Yang2020], and recover biodiversity [@Chazdon2020; @Xie2019; @Baumann2020].
...
The controversy over the biodiversity value of abandonment is strongest in Europe, where conservation often focuses on "farmland biodiversity," species that depend on open habitats and low-intensity farmland as a result of Europe's long history of cultivation, loss of natural disturbance regimes, and cultural preferences [@Batary2015; @Fischer2008; @Fischer2012a].
In those settings, abandonment is often viewed negatively because it reduces landscape heterogeneity and the amount of open habitats, regardless of its potential to benefit other types of species [@Queiroz2014; @Navarro2012].
...
Given species' unique responses to land-use change [@Sirami2008; @Finch2019], conservationists must assess the effects of abandonment across a wide set of species and across large landscapes covering multiple habitat types.

Though many studies have investigated abandonment's effect on local biodiversity [@Bowen2007; @Sirami2008; @Uchida2014; @Plieninger2014; @Regos2016; @Isbell2019], few have investigated how abandonment affects biodiversity at the landscape scale across multiple regions of the world.
Crucially, prior studies have generally lacked the spatial and temporal resolution required to track abandonment dynamics through time.
...
Recent efforts to more accurately and precisely map abandonment from satellite imagery now provide annual maps of cropland abandonment with 30-m spatial resolution for many regions across the globe [@Yin2020]. 
Here, we combine high-resolution land-cover time series for 11 sites on four continents [@Yin2020] with range maps and habitat preferences for 1,322 species of birds and mammals to calculate changes in area of habitat (AOH, hereafter "habitat") [@Brooks2019] resulting from cropland abandonment annually from 1987 and 2017.

We address three questions: 
(i) How did cropland abandonment affect habitat availability for individual bird and mammal species, and how did the number of species that gained ("winners") or lost ("losers") habitat differ across sites and for different species traits?
...
Of the 66 threatened species occurring at our sites (Fig. \@ref(fig:richness-by-site)), `r prettyNum(round(100*p_test_iucn_all$estimate[2], 1), nsmall = 1)`% gained habitat following abandonment (Fig. \@ref(fig:aoh-iucn-by-site-crop-abn)), approximately the same proportion as for non-threatened species [`r prettyNum(round(100*p_test_iucn_all$estimate[1], 1), nsmall = 1)`%; two-sample proportion test: $X^2$ (df = `r p_test_iucn_all$parameter`, *N* = `r aoh_gain_by_iucn_tmp %>% filter(aoh_type == "crop_abn_iucn") %>% .$N %>% sum()`) = `r prettyNum(round(p_test_iucn_all$statistic, digits = 2), nsmall = 2)`, *p* `r ifelse(p_test_iucn_all$p.value < 0.01,"< 0.01", paste0("= ", round(p_test_iucn_all$p.value, digits = 2)))`].
Only nine small-ranged species (3 birds and 6 mammals with ranges smaller than their Class's global median) occurred at our sites, out of which eight gained habitat (Fig. \@ref(fig:aoh-by-range)).
...
However, the opposite pattern played out for habitat changes relative to the beginning of the time series: `r effect_size_stats_tmp$share_doubled_obs`% of winners doubled their habitat (or more), while only `r effect_size_stats_tmp$share_halved_obs`% of losers halved their habitat (or worse; Fig. \@ref(fig:log-ratio-aoh-change-by-site)).
...
Species that occupy grassland and forest habitats saw larger relative increases in habitat over time (by a factor of `r filter(trait_mod_coefs, response == "log_ratio", str_detect(term, "grassland")) %>% pull(estimate)` and `r filter(trait_mod_coefs, response == "log_ratio", str_detect(term, "for")) %>% pull(estimate)`, respectively), but species with savanna and shrubland suitability experienced smaller relative changes (reducing habitat area by factors of `r filter(trait_mod_coefs, response == "log_ratio", str_detect(term, "sav")) %>% pull(estimate)` and `r filter(trait_mod_coefs, response == "log_ratio", str_detect(term, "shrub")) %>% pull(estimate)`, respectively; Table \@ref(tab:traits-mod-table-main)).
...
Tropical species benefited more than temperate species: For each additional degree of latitude the center of a species' range is from the equator, habitat area declined by `r -1*(filter(trait_mod_coefs, response == "percent", str_detect(term, "latitude")) %>% pull(estimate))`% of site area.
...
Accordingly, we examine whether the benefits of abandonment could compensate for (i) the initial habitat cleared for the cultivation of croplands that were later abandoned (Calculation 2) or (ii) habitat loss that occurred elsewhere due to cropland expansion (Calculation 3).
...
When considering the net habitat change after incorporating these processes, the ratio of winners to losers became much less favorable (Fig. \@ref(fig:aoh-overall)): about equal numbers of mammal species and more birds lost habitat than gained it (Table \@ref(tab:main-aoh-stats-table), Figs. \@ref(fig:aoh-overall-stacked-w-pot)-\@ref(fig:aoh-overall-w-pot)).
Both declines in the share of winning species were statistically significant (see SI Section \@ref(si-prop-test-results)).

While more species lost habitat than gained it at all sites when factoring in both cropland abandonment and expansion (Fig. \@ref(fig:aoh-by-site-stacked)), these increases were most pronounced in Mato Grosso, Brazil, where less abandonment and much more cropland expansion occurred than at the other sites (Figs. \@ref(fig:aoh-by-site-max-abn) and \@ref(fig:aoh-by-site-full)).
Our sites in Bosnia & Herzegovina; Chongqing, China; Goiás, Brazil; and Iraq also had unfavorable ratios of winners to losers after accounting for cropland expansion, but the other sites had narrowly favorable ratios (particularly for mammal species).
