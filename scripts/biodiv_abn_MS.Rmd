...
Our study, the first multi-continental analysis using annual high-resolution land cover maps to assess the biodiversity impacts of cropland abandonment, shows that abandonment *per se* increases habitat for most bird and mammal species (Fig. \@ref(fig:aoh-by-site-crop-abn-prop)).
...
For example, our models indicate that grassland suitability has a stronger association with gains in area of habitat following abandonment than forest suitability, even though both are associated with positive trends.
...
One review of existing studies, [@Queiroz2014] most of which focus on Europe, showed that most site-specific studies reported that abandonment had negative effects on biodiversity overall, but outcomes varied substantially by taxon, biodiversity metrics, agricultural system, and how biodiversity changes were framed.
...
The three tropical sites (Figs. \@ref(fig:global-map-w-wisc)a and \@ref(fig:site-biomes)), showed the largest overall benefits from abandonment with the highest absolute numbers of species gaining area of habitat, led by Mato Grosso and Goiás, Brazil (Table \@ref(tab:aoh-stats-by-site); Fig. \@ref(fig:aoh-by-site-crop-abn)).
...
We analyzed changes in landscape configuration at our sites over time and found that while abandonment increased grassland and forest area overall, those land cover types became less aggregated over time, while cropland became more aggregated.
We note however, that an increase in overall fragmentation arising from regenerating croplands is biologically distinct from an increase stemming from the active destruction of existing areas of forest and grassland.
...
These sites ranged from `r round(min(area_summary_df$total_site_area_ha_2017/10^6), digits = 2)` million to `r round(max(area_summary_df$total_site_area_ha_2017/10^6), digits = 2)` million ha (Mha) in area.
...
Pasture abandonment is much harder to map accurately from satellite imagery because it requires distinguishing between grasslands that are actively managed for livestock grazing (pastures, grazed rangelands, etc.), formerly managed but abandoned grasslands, and undisturbed natural grasslands subject to non-livestock grazing.
...
The proportion of species with significant gains in AOH is show by taxonomic order (Figs. \@ref(fig:trend-by-order)) and by family (Figs. \@ref(fig:trend-by-family-birds)-\@ref(fig:trend-by-family-mammals).
...
## Landscape configuration and habitat fragmentation {.unnumbered}
