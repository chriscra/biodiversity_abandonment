...
rmarkdown::render(
  ...
  output_file = paste0(p_proj, "writing/previews/", "Crawford_bd_abn_MS_draft",
...
# 3468 After latest round 2/2/2023
184 #abstract
...
knitr::include_graphics(path = paste0(p_proj, "writing/fig_images/wisconsin_example_w_map_v1.pdf"))
```
...
aoh_table_tmp1 %>%
  ...
  select("AOH Calculation" = aoh_type, "Vert. Class" = vert_class,
         ...
         # "Win/Loss Ratio (incl. weak trends)" = win_loss_weak
         ) %>%
  # {if (doc_type == "word") 
    flextable(.) %>% 
      theme_booktabs(.) %>%
...
Though few threatened species occurred at our sites (Fig. \@ref(fig:richness-by-site)), most gained habitat following abandonment (Fig. \@ref(fig:aoh-iucn-by-site-crop-abn)), though not disproportionately more or less than for non-threatened species [two-sample proportion test: $X^2$ (df = `r p_test_iucn_all$parameter`, *N* = `r aoh_gain_by_iucn_tmp %>% .$N %>% sum()`) = `r prettyNum(round(p_test_iucn_all$statistic, digits = 2), nsmall = 2)`, *p* `r ifelse(p_test_iucn_all$p.value < 0.01,"< 0.01", paste0("= ", round(p_test_iucn_all$p.value, digits = 2)))`].
...
Many abandoned croplands were recultivated during our time series (38.05% on average across sites @Crawford2022), with substantial impacts on species’ AOH. Had no recultivation taken place during our time series, the percentage of consistent winners would have increased `r paste0("from ", aoh_stats_main$n_gain_bird_percent, "% to ", obs_v_pot_stats$n_gain_bird_pot_percent, "% for birds and ", aoh_stats_main$n_gain_mam_percent, "% to ", obs_v_pot_stats$n_gain_mam_pot_percent, "% for mammals")` (Table \@ref(tab:main-aoh-stats-table), Fig. \@ref(fig:aoh-obs-v-pot-by-site)), both statistically significant increases (see SI).
...
Both declines in the share of winning species were statistically significant (see SI).
...
First, our analysis is based on remotely-sensed land cover interpolated to specific IUCN habitat types, which do not provide a reliable proxy for habitat quality, particularly when quality is related to regeneration time. 
...
Because this crosswalk delineates "one to many" relationships (i.e., one land cover class could be multiple IUCN habitat types), we next assigned individual pixels of land cover to the most common corresponding IUCN habitat type within the surrounding pixels in the Jung et al. [@Jung2020a] habitat map (see SI).
...
We extracted trophic level and body mass variables from a recent database of species traits for mammals and birds [@Etard2020] and combined them with variables derived from individual IUCN assessments, including habitat preferences (suitability of various IUCN Level 1 habitats), habitat breadth (counting the number of suitable IUCN Level 2 habitats, following Etard et al. @Etard2020), vertebrate class, threat status, global range size, and tropicality (measured as the absolute latitude of the centroid of a species range).
...
We thank the Macaulay Library at the Cornell Lab of Ornithology for providing access to the photos in Fig. \@ref(fig:global-map-w-wisc).
