...
Abandoned lands represent opportunities to restore ecosystems yet variation in the distribution of species and habitats, along with a lack of detailed information on abandonment, have resulted in controversy over whether abandonment will benefit, or harm, biodiversity.
Here, we use annual land cover maps to estimate habitat changes for 2023 bird and mammal species at 11 sites across 4 continents for 1987-2017.
...
However, frequent recultivation attenuated these benefits: more species would have benefited (`r abstract_stats$percent_winner_bird_pot`% and `r abstract_stats$percent_winner_mam_pot`% respectively) if recultivation had not occurred.
Furthermore, a plurality of birds (`r abstract_stats$percent_loss_bird_full_obs`%) and about equal numbers of mammals (`r abstract_stats$percent_loss_mam_full_obs`%) still experienced net habitat loss after accounting for land clearing for agriculture prior to or alongside abandonment, despite gains from abandonment.
...
Habitat loss through the clearing of land for agriculture remains the most widespread threat to terrestrial biodiversity [@Tilman2017], and agricultural expansion continues to threaten many ecosystems [@Potapov2022].
Yet agricultural land use change is not unidirectional: amidst continued cropland expansion, hundreds of millions of hectares of agricultural lands have been abandoned around the world since 1950 due to socioeconomic, demographic, and technological changes [@ReyBenayas2007; @Li2017; @Potapov2022; @Chazdon2020].
As agriculture intensifies and mechanizes and rural outmigration, urbanization, and globalization continue, some researchers expect abandonment to accelerate [@Sanderson2018; @Taylor2021], and environmentalists increasingly look to former agricultural lands for low-cost opportunities to restore ecosystems [@Navarro2012], sequester carbon [@Griscom2017; @Yang2020], and recover biodiversity [@Chazdon2020; @Xie2019; @Baumann2020].

Importantly, however, biodiversity is not guaranteed to benefit when cropland is abandoned.
Conceptually, spatial and temporal variation in abandonment substantially affects biodiversity outcomes since species have unique biophysical and habitat requirements.
Practically, the recovery of natural ecosystems typically requires multiple decades [@Acevedo-Charry2019; @Isbell2019; @Nerlekar2020].
If succession is interrupted by recultivation, as recent evidence suggests frequently occurs within a decade or two, these sites are unlikely to accumulate substantial amounts of carbon or biodiversity [@Crawford2022; @Dara2018].
...
This controversy is perhaps most apparent in Europe, where a substantial amount of conservation attention is placed on "farmland biodiversity:" species that are often dependent on open-habitats and low-intensity farmland as a result of Europe's long history of cultivation, the loss of natural disturbance regimes, and cultural preferences [@Batary2015; @Fischer2008; @Fischer2012a].
Here, abandonment is often considered a negative phenomenon that reduces landscape heterogeneity and the amount of open habitats, regardless of the potential for gains for species that could benefit from abandonment [@Queiroz2014; @Navarro2012].
In contrast, abandonment is often viewed positively in Central and South America, where conservationists welcome it as an opportunity for forest habitats to recover [@Queiroz2014].
Given that species have unique responses to land use change [@Sirami2008; @Finch2019], conservationists need a more expansive understanding of how impacts balance out across a wide set of species and across large landscapes covering multiple habitat types.

Though many studies have investigated the effect of abandonment on local biodiversity [@Bowen2007; @Sirami2008; @Uchida2014; @Plieninger2014; @Regos2016; @Isbell2019], few have investigated how abandonment affects biodiversity at the landscape scale across multiple regions of the world by combining spatially-explicit biodiversity data with maps of abandonment.
Crucially, existing studies have generally lacked the spatial and temporal resolution required to track abandonment dynamics through time.
Given the highly dynamic nature of abandonment and the importance of regeneration time for ecosystem recovery, these details are critical to understanding the implications for biodiversity.

Recent efforts to improve abandonment mapping from satellite imagery have produced annual maps of cropland abandonment with higher spatial resolution (30-m resolution) and temporal coverage (e.g., annual intervals) for more regions than previously available [@Yin2020]. 
These new time series data have enabled detailed analyses of the long-term persistence of abandoned cropland [@Crawford2022], and also support investigations into how abandonment affects individual species across multiple regions and longer time horizons.

Here, we combine high-resolution land-cover time series for 11 sites on four continents [@Yin2020] with range maps and habitat preferences for 2023 species of birds and mammals to calculate changes in area of habitat (AOH) [@Brooks2019] resulting from cropland abandonment annually between 1987 and 2017.
We address three questions: 
(i) How did cropland abandonment affect habitat availability for individual bird and mammal species, and how does the balance of habitat gaining species ("winners") and habitat losing species ("losers") vary across sites and a range of species traits?
(ii) To what extent did recultivation limit habitat gains following abandonment?
(iii) Finally, did changes in area of habitat resulting directly from abandonment compensate for broader land use changes that occurred a) prior to cultivation and subsequent abandonment or b) elsewhere in the site?
...
Overall, we found that most bird (`r aoh_stats_main$n_gain_bird_percent`%) and mammal (`r aoh_stats_main$n_gain_mam_percent`%) species at our sites consistently gained habitat from abandonment, experiencing statistically significant (p < 0.05) and positive trends in AOH everywhere they occurred (Table \@ref(tab:main-aoh-stats-table), Fig. \@ref(fig:aoh-by-site-crop-abn-prop)).
...
However, some species exhibited contrasting trends at different sites: `r aoh_stats_main$n_w_gain_bird_percent`% of birds and `r aoh_stats_main$n_w_gain_mam_percent`% of mammals gained habitat at some sites but showed no trend (p > 0.05) at others (i.e., “weak” gains), while `r aoh_stats_main$n_w_loss_bird_percent`% and `r aoh_stats_main$n_w_loss_mam_percent`% respectively lost habitat at some sites but not others (i.e., “weak” losses; Table \@ref(tab:main-aoh-stats-table)).
The number of individual species-site trends are shown in Table \@ref(tab:aoh-stats-table-all-obs).

The ratio of winners to losers varied across sites, but remained consistently greater than one (Fig. \@ref(fig:aoh-by-site-crop-abn-prop), Table \@ref(tab:aoh-stats-by-site)).
The lone exception involved birds in Iraq (the site with the lowest species richness; Fig. \@ref(fig:richness-by-site)), where `r 100*(1-aoh_stats_main$wl_iraq)`% more species lost habitat than gained it (Figs. \@ref(fig:aoh-by-site-crop-abn-prop) and \@ref(fig:aoh-by-site-crop-abn)).
...
Though few threatened species occurred at our sites (Fig. \@ref(fig:richness-by-site)), `r round(100*p_test_iucn_all$estimate[2], 2)`% gained habitat following abandonment (Fig. \@ref(fig:aoh-iucn-by-site-crop-abn)), though not disproportionately more or less than did non-threatened species [`r round(100*p_test_iucn_all$estimate[1], 2)`%; two-sample proportion test: $X^2$ (df = `r p_test_iucn_all$parameter`, *N* = `r aoh_gain_by_iucn_tmp %>% .$N %>% sum()`) = `r prettyNum(round(p_test_iucn_all$statistic, digits = 2), nsmall = 2)`, *p* `r ifelse(p_test_iucn_all$p.value < 0.01,"< 0.01", paste0("= ", round(p_test_iucn_all$p.value, digits = 2)))`].
Only nine small-ranged species (3 birds and 6 mammals with ranges smaller than the global median) occurred at our sites; all species except one bird gained habitat following abandonment (Fig. \@ref(fig:aoh-by-range)).
...
However, the opposite pattern played out for relative changes in AOH at the end of our study period compared to the beginning: `r effect_size_stats_tmp$share_doubled_obs`% of winners doubled their habitat (or more), while only `r effect_size_stats_tmp$share_halved_obs`% of losers halved their habitat (or worse; Fig. \@ref(fig:log-ratio-aoh-change-by-site)).

(ref:caption-aoh-by-site-crop-abn-prop) Species’ responses to cropland abandonment shown as the proportion of bird and mammal species with various trends in area of habitat (AOH). Habitat changes are calculated by tracking habitat provided by abandoned croplands from the year of first cultivation through 2017 (Calculation 1, see Methods). The number of bird and mammal species at each site is listed in x-axis labels, and the total number of species across all sites is shown at far left ("Overall"). The number of species in each trend at each site are shown in Fig. \@ref(fig:aoh-by-site-crop-abn). Observed and modeled AOH trends for individual species are shown in Figs. \@ref(fig:aoh-sp-obs-crop-abn) and \@ref(fig:aoh-trend-lines-crop-abn). Trends are labeled according to the conventions listed in Table 1.
...
(ref:caption-aoh-change-percent-by-trend) The distribution of changes in area of habitat (AOH) as a percent of site area, shown according to trend. Change in AOH is calculated as the difference in mean AOH during the first five years and the last five years of the time series. Distributions at each site are shown in Fig. \@ref(fig:aoh-change-percent-by-site).
...
We found that individual species habitat preferences (i.e., which habitats can be occupied by a species and are therefore considered "suitable" habitats, which in turn determine a species’ area of habitat) had a significant effect on abandonment responses.
The ability to occupy grassland, forest, or savanna habitats all increased the odds of a species gaining AOH, with grassland suitability (i.e., the ability to occupy abandoned croplands that return to grassland) having the most substantial effect, increasing the odds of being a winner by a factor of `r filter(trait_mod_coefs, response == "binary", str_detect(term, "grassland")) %>% pull(estimate)`, compared to `r filter(trait_mod_coefs, response == "binary", str_detect(term, "forest")) %>% pull(estimate)` and `r filter(trait_mod_coefs, response == "binary", str_detect(term, "savanna")) %>% pull(estimate)` for forest and savanna suitability, respectively (Table \@ref(tab:traits-mod-table-main)).
Grassland and savanna suitability were also associated with larger increases in AOH overall: on average, species that can occupy grasslands and savannas gained an additional area of habitat equal to `r filter(trait_mod_coefs, response == "percent", str_detect(term, "grassland")) %>% pull(estimate)`% and `r filter(trait_mod_coefs, response == "percent", str_detect(term, "sav")) %>% pull(estimate)`% of site area, respectively.
(An AOH increase of `r filter(trait_mod_coefs, response == "percent", str_detect(term, "grassland")) %>% pull(estimate)`% of site area corresponds to an increase in absolute area of `r filter(trait_mod_coefs, response == "percent", str_detect(term, "grassland")) %>% pull(estimate)` percentage points, such as a hypothetical increase from 10% to `r filter(trait_mod_coefs, response == "percent", str_detect(term, "grassland")) %>% pull(estimate) + 10`% or 2% to `r filter(trait_mod_coefs, response == "percent", str_detect(term, "grassland")) %>% pull(estimate) + 2`% of site area.)
Species that occupy grassland and forest habitats saw larger relative increases in AOH over time (increasing by a factor of `r filter(trait_mod_coefs, response == "log_ratio", str_detect(term, "grassland")) %>% pull(estimate)` and `r filter(trait_mod_coefs, response == "log_ratio", str_detect(term, "for")) %>% pull(estimate)` respectively), but species with savanna and shrubland suitability experienced smaller relative changes (reducing the relative AOH change by `r 100 * (1-filter(trait_mod_coefs, response == "log_ratio", str_detect(term, "sav")) %>% pull(estimate))`% and `r 100 * (1-filter(trait_mod_coefs, response == "log_ratio", str_detect(term, "shrub")) %>% pull(estimate))`% respectively, Table \@ref(tab:traits-mod-table-main)).
...
Species capable of using a greater number of habitats and species with larger ranges saw larger increases in AOH, with species' AOH increasing by `r filter(trait_mod_coefs, response == "percent", str_detect(term, "n_suitable")) %>% pull(estimate)`% of site area for each additional habitat type they could occupy and `r filter(trait_mod_coefs, response == "percent", str_detect(term, "range")) %>% pull(estimate)`% of site area for each order of magnitude increase in range size. 
Tropical species also benefited more than temperate species: for each additional degree of latitude the center of a species' range is from the equator, AOH declined by `r -1*(filter(trait_mod_coefs, response == "percent", str_detect(term, "latitude")) %>% pull(estimate))`% of site area.
Range area and latitude were also associated with larger relative gains in AOH.
...
Many abandoned croplands were recultivated during our study period (38.05% on average across sites @Crawford2022), with substantial impacts on species’ AOH.
Had no recultivation occurred, the percentage of consistent winners would have increased `r paste0("from ", aoh_stats_main$n_gain_bird_percent, "% to ", obs_v_pot_stats$n_gain_bird_pot_percent, "% for birds and ", aoh_stats_main$n_gain_mam_percent, "% to ", obs_v_pot_stats$n_gain_mam_pot_percent, "% for mammals")` (Table \@ref(tab:main-aoh-stats-table), Fig. \@ref(fig:aoh-obs-v-pot-by-site)); both were statistically significant increases (see SI Section \@ref(si-prop-test-results)).
While the number of winners would have increased, so too would the number of losers, from `r aoh_stats_main$n_loss_bird_percent`% to `r obs_v_pot_stats$n_loss_bird_pot_percent`% for birds and `r aoh_stats_main$n_loss_mam_percent`% to `r obs_v_pot_stats$n_loss_mam_pot_percent`% for mammals (Table \@ref(tab:main-aoh-stats-table)).

Species in Mato Grosso were most dramatically affected by recultivation, where an additional `r filter(n_aoh_trend_changes_by_site_df, site == "mato_grosso")$gain` species gained AOH and an additional `r filter(n_aoh_trend_changes_by_site_df, site == "mato_grosso")$loss` lost AOH.
Aside from Chongqing (`r filter(n_aoh_trend_changes_by_site_df, site == "chongqing")$gain` additional winners, `r filter(n_aoh_trend_changes_by_site_df, site == "chongqing")$loss` additional losers) and Nebraska / Wyoming (`r filter(n_aoh_trend_changes_by_site_df, site == "nebraska")$gain` additional winners), all other sites experienced fewer than 3 additional winners or losers (Fig. \@ref(fig:aoh-obs-v-pot-by-site), Table \@ref(tab:aoh-changes-obs-v-pot)).
Preventing recultivation also would have amplified the magnitude of AOH changes, with both winners gaining and losers losing more habitat (Figs. \@ref(fig:aoh-p-change-obs-v-pot-bird)-\@ref(fig:aoh-p-change-obs-v-pot-mam)).
...
Thus far, our analysis has focused solely on the habitat provided by cropland that was abandoned (i.e., during cultivation and subsequent abandonment), without considering the original land cover at the beginning of our study period (Calculation 1, Table \@ref(tab:main-aoh-stats-table), Fig \@ref(fig:aoh-sample-trajectories)).
Though abandonment benefitted many more species than it harmed (Table \@ref(tab:main-aoh-stats-table)), did it compensate for (i) the amount of habitat that was cleared for croplands that were abandoned later on (i.e., the net change in abandoned croplands relative to the start of the time series; Calculation 2) or (ii) the amount of habitat loss that occurred elsewhere due to cropland expansion (Calculation 3)?
When considering the net AOH change in abandoned croplands across the entire temporal extent or throughout the entire spatial extent at each site, the ratio of winners to losers becomes much less favorable (Fig. \@ref(fig:aoh-overall)): more bird species lost habitat than gained it, and about equal numbers of mammals lost as gained, maintaining a ratio of winners to losers only marginally greater than one (Table \@ref(tab:main-aoh-stats-table), Figs. \@ref(fig:aoh-overall-stacked-w-pot) and \@ref(fig:aoh-overall-w-pot)).
Both declines in the share of winning species were statistically significant (see SI Section \@ref(si-prop-test-results)).

While the number of losing species increased at all sites when factoring in both cropland abandonment and expansion (Fig. \@ref(fig:aoh-by-site-stacked)), these increases were most acute in Mato Grosso, Brazil, where less cropland abandonment and much more cropland expansion occurred than at other sites (see site-specific results in Figs. \@ref(fig:aoh-by-site-max-abn) and \@ref(fig:aoh-by-site-full)).
Our sites in Bosnia & Herzegovnia; Chongqing, China; Goiás, Brazil; and Iraq also showed ratios of winners to losers that were less than one when factoring in cropland expansion, but sites elsewhere managed to narrowly maintain favorable ratios (particularly for mammal species), despite these ratios declining overall.
A greater share of threatened species also lost habitat when factoring in cropland expansion that occurred prior to or alongside abandonment, but not statistically more so than non-threatened species (Fig. \@ref(fig:aoh-by-iucn-lumped)).
Limiting recultivation would have reduced the number of losing species in both of these broader AOH calculations, and even would have resulted in a favorable ratio of winners to losers even after factoring in habitat clearing for agriculture prior to abandonment (Calculations 2 and 3; Figs. \@ref(fig:aoh-overall-stacked-w-pot) and \@ref(fig:aoh-overall-w-pot)).
Limiting recultivation would have had a similar but much smaller effect when considering AOH changes through the entire spatial extent (Calculation 3), however.

(ref:caption-aoh-overall) The number of bird and mammal species with various trends in area of habitat (AOH) across our 11 study sites from 1987-2017. Columns correspond to three different AOH calculations: 1) AOH changes directly attributable to cropland abandonment (from first cultivation through 2017), accounting for habitat provided by abandoned croplands during cultivation and subsequent abandonment and recultivation; 1b) AOH changes from abandonment, assuming no recultivation; 2) net AOH changes in abandoned croplands across the entire temporal extent (1987-2017), accounting for habitat cleared for agriculture prior to abandonment; and 3) AOH changes across the entire spatial extent at each site (1987-2017). Results for all AOH calculations are shown as proportions in Fig. \@ref(fig:aoh-overall-stacked-w-pot) and raw counts in Fig. \@ref(fig:aoh-overall-w-pot). Site-specific proportions are shown in Fig. \@ref(fig:aoh-by-site-stacked) and raw counts are shown for AOH calculations 1b, 2, and 3 in Figs. \@ref(fig:aoh-by-site-crop-abn-potential), \@ref(fig:aoh-by-site-max-abn), and \@ref(fig:aoh-by-site-full), respectively.
...
The growing trend of global cropland abandonment has reshaped agricultural landscapes around the world.
Our study, the first multi-continental assessment of the biodiversity impacts of cropland abandonment, shows that abandonment *per se* increases habitat for the majority of bird and mammal species at our 11 study sites (Fig \@ref(fig:aoh-by-site-crop-abn-prop)).
These gains can be largely explained by species' habitat preferences, such that species that can occupy forests and grasslands were more likely to gain habitat, and species that occupy arable land were most likely to lose habitat. 
While this follows directly from how area of habitat is calculated, it reflects how abandonment affects the availability of specific habitat types, and in turn which species are affected.
Species with larger ranges, ranges centered closer to the equator, or capable of utilizing a greater number of habitats were more likely to gain habitat following abandonment.
Overall, a greater share of bird species experienced declines in AOH following abandonment than mammals (Fig \@ref(fig:aoh-by-site-crop-abn-prop), Table \@ref(tab:main-aoh-stats-table)), though a majority of both birds and mammals still gained habitat overall.
The overall proportion of winners did not differ significantly for threatened compared to non-threatened species (Fig. \@ref(fig:aoh-iucn-by-site-crop-abn)) nor for large- compared to small-ranged species (Fig. \@ref(fig:aoh-by-range)).
From a conservation perspective, it is encouraging that the majority of these species, which are often considered to be of greater conservation concern, experienced gains in AOH.
...
However, habitat gains from abandonment were not uniform across species and sites.
Moreover, our analysis suggested that abandonment rarely compensated for habitat lost due to cropland expansion -- regardless of whether these newly cultivated croplands were subsequently abandoned or not.
Most concerningly, habitat gains from abandonment were dampened by frequent recultivation of abandoned croplands: the ephemerality of abandonment reduced both the overall share of species with increasing trends (Figs. \@ref(fig:aoh-overall) and \@ref(fig:aoh-obs-v-pot-by-site), Table \@ref(tab:aoh-changes-obs-v-pot)) and the magnitude of AOH gained per species during our study period (Fig. \@ref(fig:aoh-change-percent-by-trend-obs-v-pot)).
Despite the clear potential for abandonment to generate habitat for bird and mammal species, expectations for abandonment to produce overall gains are tempered by the fact that most landscapes are still experiencing habitat loss coupled with recultivation.

Our sites cover nine different biomes, where differences in species communities, species richness, and the distributions of habitats produced unique responses to abandonment (Fig. \@ref(fig:site-biomes)).
Though we only had three sites in the tropics (Figs. \@ref(fig:global-map-w-wisc)a and \@ref(fig:site-biomes)), these sites showed the highest number of winning species, led by Mato Grosso and Goiás, Brazil (Table \@ref(tab:aoh-stats-by-site); Fig. \@ref(fig:aoh-by-site-crop-abn)).
Tropical regions showed the largest overall benefits from abandonment, measured in terms of absolute numbers of species gaining AOH, in part due to high species richness in the tropics, but also due to the high sensitivity of many tropical species to habitat modification (i.e., a smaller fraction of the tropical species present at our sites are able to persist in artificial habitats compared to temperate species; see Figs. \@ref(fig:p-arable-suitability) and \@ref(fig:latitude-v-arable-suitability)).
However, we note that Mato Grosso experienced the least amount of cropland abandonment of any site and thus remained dominated by habitat loss, which led to relatively small gains in AOH (one or two orders of magnitude less for individual species than other sites, whether area is measured in absolute terms or as a percent of site extent; Fig. \@ref(fig:aoh-sp-obs-crop-abn)) and the greatest share of species with losing trends when accounting for land cover changes throughout the site (Fig. \@ref(fig:aoh-by-site-stacked)).
...
This could reflect the relative refuge provided by croplands amidst a harsh desert environment or Iraq's long history of human cultivation [@Balter2007; @Batary2015].
...
However, these outcomes substantially varied by taxonomic focus, biodiversity metrics, agricultural system, and whether biodiversity changes were framed in the context of  pre-abandonment losses or post-abandonment gains.
Research perspectives on abandonment also varied by region (with predominantly negative views of abandonment in Eurasia), potentially the result of differences in conservation focus (e.g., which species are considered, and at what scale), or geographic differences in biodiversity outcomes [@Queiroz2014].

Our multi-continental analysis indeed echoes the heterogeneous species responses shown in prior studies [@Sirami2008;@Regos2016], but it provides a more positive view of abandonment overall.
...
Moreover, the benefits of abandonment are typically overshadowed by cropland expansion at each site and by recultivation of abandoned sites, which our results show can produce overall declines despite compensatory gains from abandonment.
Though habitat regeneration following abandonment would benefit most birds and mammals at almost all of our sites, species that stand to lose habitat from this approach should be monitored, and when necessary, incorporated into conservation planning efforts to ameliorate habitat losses, with particular attention given to the maintenance of high-biodiversity open habitats that support threatened or rare species.
...
A few limitations to this study should be noted.
First, our analysis is based on remotely-sensed land cover interpolated to specific habitat types as delineated by IUCN [@IUCN2021] and BirdLife International [@BirdLife2021], which do not provide a reliable proxy for habitat quality, particularly when quality is related to post-abandonment regeneration time (e.g., not allowing for distinctions between intact old-growth forests and young secondary forests).
...
However, if post-abandonment successional habitats are protected from recultivation, these species are also likely to benefit from abandonment, while species characteristic of earlier successional stages are likely to decline or disappear, highlighting the dynamic nature of communities.
...
These finer-grained vegetation characteristics, in turn, combined with differences in dispersal ability among bird and mammal species will affect which species of birds and mammals actually occupy and persist at a given abandoned site.
...
Third, while birds and mammals are commonly used as general indicators for biodiversity [@Westgate2017], other taxonomic groups may not respond in the same ways, particularly following the abandonment of certain types of croplands.
...
We excluded amphibians and reptiles from our study due to greater uncertainty about their distributions and habitat requirements, but these groups offer promising opportunities for future work.
Finally, although our study sites covered 4 continents, they were restricted in area, and we therefore could not assess how cropland abandonment affected habitat availability across the full range of any species; this whole-range information would be required to estimate changes in global extinction risk and population trend over time.

By connecting high-resolution time-series data on cropland abandonment with species-specific distribution and habitat data for the first time, we show that cropland abandonment produced substantial gains in habitat for a majority of bird and mammal species over a 30-year period in 11 sites spread over four continents.
After accounting for the unique responses of individual species, we show that abandonment is unlikely to disproportionately harm species of conservation concern, and instead represents an opportunity to increase habitat availability for most birds and mammals.
Yet despite these encouraging results, our analysis of abandonment in the context of cropland expansion and broader land cover changes reinforce an inconvenient and ugly truth in conservation: Habitat loss due to agriculture remains a fundamental threat to species around the world.
Even in regions with substantial amounts of cropland abandonment, potential gains in habitat are typically outweighed by habitat losses due to cropland expansion.
In order to unlock the conservation potential of abandoned croplands, policy makers will need to create incentives and community initiatives to prevent or postpone the recultivation of abandoned croplands and sustain and enhance habitat gains, while also working to prevent further losses of natural and semi-natural ecosystems.
Doing so will be critical to bending the curve of biodiversity loss.
...
Calculation 2 considered changes in habitat that took place exclusively in pixels that experienced abandonment at some point during the time series (following Calculation 1), but now measured for the entire temporal extent of our time series, from 1987 through 2017.
...
Finally, we calculated AOH for each species across the entire temporal extent (1987-2017) and the entire spatial extent at each site ("Calculation 3;" Figs. \@ref(fig:lc-1987)-\@ref(fig:lc-2017)).
