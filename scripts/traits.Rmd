...
```{r test-lvl2-suitability}
habitat_age_req_coded %>%
  filter(str_detect(suitable_habitats, regex("forest", ignore_case = TRUE)),
         str_detect(suitable_habitats, regex("heavily degraded", ignore_case = TRUE))) #%>%
  # select(suitable_habitats) %>% unique()
...
habitat_prefs %>%
  ...
  select(binomial, #code, name,# majorImportance:suitability,
         Coarse_Name) %>% unique() %>%
  group_by(binomial) %>%
...
habitat_prefs %>%
  filter(binomial == "Odocoileus virginianus") %>%
  select(binomial, code) %>%
  unique() %>%
  separate(code, into = c("lvl1", "lvl2"), sep = "\\.", extra = "merge") %>%
    select(binomial, lvl1) %>% unique() %>% arrange(lvl1)

etard_updated %>% 
  filter(binomial == "Odocoileus virginianus") %>%
  select(binomial, Habitat_breadth_IUCN)


habitat_prefs %>% #select(binomial, code) %>% #unique() %>% arrange(code) %>%
  separate(code, into = c("lvl1", "lvl2"), sep = "\\.", extra = "merge") %>%
  select(binomial, lvl1) %>% unique() %>% mutate(lvl1 = as.integer(lvl1)) %>%
  arrange(lvl1) %>%
  group_by(binomial) %>%
  summarise(n_lvl1_habitats = n(), lvl1_habitats = str_flatten(lvl1, collapse = ", "))


aoh_obs_change_tmp %>%
  select(aoh_type:binomial, trend) %>%
...
habitat_age_req_coded %>%
  filter(binomial %in% unique(aoh_ms_tmp_trends_by_sp$binomial)) %>%
  select(vert_class, binomial, common_names, suitable_habitats) %>%
  mutate(degraded_forest_occ = str_detect(suitable_habitats, regex("plantations|degraded", ignore_case = TRUE))) %>%
  filter(degraded_forest_occ)
...
# Site & Species controls

```{r test-site-sp-controls}
# ---------------------------------------------------------------------------------------------- #
# 1c.--------------------- binary model where 1 is gain and 0 is loss -------------------------- #
# ---------------------------------------------------------------------------------------------- #
names(aoh_obs_change_tmp)
...
# data:
aoh_obs_change_tmp # see _util_files.R, this is just for "crop_abn_iucn"

binary_mod_site_sp_id <-
  glm(binary_gain_v_loss ~
        site + binomial +
        forest_occ + savanna_occ + shrubland_occ + grassland_occ + wetlands_occ + 
        rocky_occ + caves_occ + desert_occ + urban_occ + arable_occ + 
        n_suitable_habitats_lvl2 + vert_class + 
        threatened + Trophic_level + 
        log10(Body_mass_g) + log10(total_range_area) + abs(centroid_latitude) + 
        max_abn_ext_percent_site,
      data = aoh_obs_change_tmp,
...
c("binomialAccipiter nisus", "binomialAccipiter soloensis", "binomialAccipiter virgatus",
  "binomialAcridotheres cristatellus", "binomialAcrocephalus bistrigiceps") %>%
  str_extract(., "binomial")

binary_mod_site_sp_id %>%
  tidy() %>% mutate(id = 1:nrow(.)) %>% arrange(desc(id))
  print(n = 30) # %>%
  mutate(site = str_extract(term, "^site[a-z,_]*") %>% gsub("site", "", .),
         # term = gsub(" ", "_", term),
         sit = str_detect(term, "^site"),
         bin = str_detect(term, "^binomial"),
         binomial = str_replace(term, "binomial", "")) %>%
  arrange(sit, bin)


binary_mod_site_sp_id %>%
    tidy() %>%
    mutate(OR = exp(1)^estimate, # exponentiate coefs so that they correspond to Odds Ratios
           site = str_extract(term, "^site[a-z,_]*") %>% gsub("site", "", .),
           binomial = str_extract(term, "binomial[a-z]") %>%
             gsub("binomial", "", .),
           # common = str_extract(term, ".[0-9]+.[0-9]+.")
           ) %>% 
  # arrange(site, binomial) %>%
  print(n = 50)



aoh_obs_change_tmp %>% group_by(Trophic_level) %>% summarise(n = n())
aoh_obs_change_tmp %>% group_by(binary_gain_v_loss) %>% summarise(n = n())
...
# load the phylogenetic trees for use in phyloglm() and phylolm()
bt <- read.tree("/Users/christophercrawford/Downloads/mnt/data/projects/birdphylo/Tree_sets/Stage2_full_data/CombinedTrees/AllBirdsHackett1.tre")


# produce consensus trees from a sample of many trees:
...
aoh_obs_change_tmp %>%
  select(aoh_type:mature_forest_obl)
...
phylo_sp %>% 
  write_csv(file = paste0(p_derived, "phylogeny_species_list.csv"))

vl <- read_csv("/Users/christophercrawford/Downloads/vertlife_taxonomies.csv")

vl %>% select(group) %>% unique
vl %>% filter(group %in% c("Birds", "Mammals"))

phylo_sp %>%
  left_join(vl %>% filter(group %in% c("Birds", "Mammals")) %>% mutate(test = 1), # VertLife species list
            by = c("binomial" = "scientificname")
            ) %>%
  filter(is.na(test)) %>%
  group_by(vert_class) %>% summarise(n = n())

vl %>%
  filter(str_detect(scientificname, "flammea"))
...
# calculate the number of sites each species occurrs in:
aoh_obs_change_tmp %>% 
  select(aoh_type, vert_class, site, binomial) %>%
  group_by(binomial) %>%
  summarise(#p_gain = mean(overall_trend == "gain"),
...
glm_mod_by_sp_df %>%
  ...
  text_transform(
    locations = cells_body(),
    fn = function(x) {str_replace_all(x, pattern = "km2", replacement = "km<sup>2</sup>")}) %>%
  gtsave(filename = paste0(p_plots, "aoh/traits/", "Table1a_binary_est", mod_label,".png"))
