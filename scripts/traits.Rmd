...
etard_update_iucn_syn <-
  ...
  # select(etard_id:binomial_etard) %>%
  # head() %>%
...
etard_update_etard_syn <-
  ...
  # select(etard_id:binomial_etard) %>%
  # head() %>%
  left_join(etard_synonyms_tmp, by = c("binomial_etard" = "Accepted")) %>%
...
```{r set-up-tmp-files}
# ---------------------------------------------- #
# setting up the following files:
# aoh_mod_tmp_trends_by_sp # categorical trends by species (one trend per species)
# aoh_est_change_tmp # estimated change in AOH based on ols regression with NW estimator
# aoh_obs_change_tmp # observed change in AOH based on mean values for start and end of time series (for various window sizes)
# ---------------------------------------------- #


# ---------------------------------------------- #
# Categorical trends in AOH
# ---------------------------------------------- #
aoh_mod_tmp_trends_by_sp <-
  ...
  left_join(select(etard_updated, -c(Order, Family, Genus)),
...
# ---------------------------------------------- #
# Estimated changes from model trends 
# For response variables:
# Absolute change in AOH, and scaled to site, and most recently, 
# binary trends
# ---------------------------------------------- #
aoh_est_change_tmp_all <-
  ...
  left_join(aoh_mod_tmp_trends_by_sp %>% select(binomial, total_range_area)) %>%
  left_join(centroids_df) %>%
  # add habitat specialities
  left_join(habitat_occurrence_df) %>%
...
# ---------------------------------------------- #
# Observed changes in AOH, for response variables e.g.,
# Ratio change in AOH
# Proportional change in AOH
# Factor change in AOH, as well as:
# 
# ---------------------------------------------- #
...
tidy(aoh_latitude_lm, conf.int = TRUE)
glance(aoh_latitude_lm)
summary(aoh_latitude_lm)
plot(aoh_latitude_lm, 1)
plot(aoh_latitude_lm, 2)
...
lm_response_variables_slope <- c("slope", "slope_prop_site")
...
glm_mod_df <- 
  ...
  mutate(
    model = map2(
      data, response, 
      ~glm(as.formula(paste0(as.character(.y), " ~ forest_occ + savanna_occ + shrubland_occ + grass_occ + wetlands_occ +
                            artificial_occ + n_hab_occ + threatened + Trophic_level + log(Body_mass_g) +
                            log(total_range_area) + abs(centroid_latitude) + max_abn_extent_div_site_area")), 
...
lm_mod_df <- 
  ...
  mutate(
    model = map2(
      data, response, 
      ~lm(as.formula(paste0(as.character(.y), " ~ forest_occ + savanna_occ + shrubland_occ + grass_occ + wetlands_occ +
                            artificial_occ + n_hab_occ + threatened + Trophic_level + log(Body_mass_g) +
                            log(total_range_area) + abs(centroid_latitude) + max_abn_extent_div_site_area")), 
          data = .x)),
    tidy = lapply(model, tidy))
...
all.equal(
  tidy(binary_mod_gain_v_loss)$estimate,
  glm_mod_df %>% filter(response == "binary_gain_v_loss", aoh_type == "crop_abn_iucn") %>% 
    pull(tidy) %>% .[[1]] %>% pull(estimate))

all.equal(
  tidy(binary_mod_loss_sp_site)$estimate,
  glm_mod_df %>% filter(response == "binary_trend_loss", aoh_type == "crop_abn_iucn") %>% 
    pull(tidy) %>% .[[1]] %>% pull(estimate))

all.equal(
  tidy(binary_mod_no_trend_sp_site)$estimate,
  glm_mod_df %>% filter(response == "binary_trend_no_trend", aoh_type == "crop_abn_iucn") %>% 
    pull(tidy) %>% .[[1]] %>% pull(estimate))


all.equal(
  tidy(abs_change_aoh_obs_lm)$estimate,
  lm_mod_df %>% filter(response == "abs_change", aoh_type == "crop_abn_iucn") %>% 
    pull(tidy) %>% .[[1]] %>% pull(estimate))

all.equal(
  tidy(abs_change_obs_aoh_div_site_area_lm)$estimate,
  lm_mod_df %>% filter(response == "abs_change_percent_site", aoh_type == "crop_abn_iucn") %>% 
    pull(tidy) %>% .[[1]] %>% pull(estimate))

all.equal(
  tidy(abs_change_obs_aoh_div_site_area_lm)$estimate,
  trait_mod_df %>% filter(response == "abs_change_percent_site", aoh_type == "crop_abn_iucn") %>% 
    pull(tidy) %>% .[[1]] %>% pull(estimate))

all.equal(
  tidy(ratio_change_aoh_lm)$estimate,
  lm_mod_df %>% filter(response == "ratio", aoh_type == "crop_abn_iucn") %>% 
    pull(tidy) %>% .[[1]] %>% pull(estimate))

all.equal(
  tidy(log_ratio_change_aoh_lm)$estimate,
  lm_mod_df %>% filter(response == "log(ratio)", aoh_type == "crop_abn_iucn") %>% 
    pull(tidy) %>% .[[1]] %>% pull(estimate))


all.equal(
  tidy(aoh_slope_lm)$estimate,
  lm_mod_df %>% filter(response == "slope", aoh_type == "crop_abn_iucn") %>% 
    pull(tidy) %>% .[[1]] %>% pull(estimate))

all.equal(
  tidy(aoh_slope_prop_site_lm)$estimate,
  lm_mod_df %>% filter(response == "slope_prop_site", aoh_type == "crop_abn_iucn") %>% 
    pull(tidy) %>% .[[1]] %>% pull(estimate)*10^4)
...
lapply(c(1,4), function(i) {
  tidy(mod_list[[i]]) %>% 
    ...
    mutate(mod = names(mod_list)[i])
}) %>% 
  # do.call(cbind, .) #%>% 
  bind_rows() %>%
  mutate(mod = gsub("binary_mod_gain", "bin_gain", mod),
         p.value = 
           case_when(p.value < 0.001 ~ "<0.001",
                     TRUE ~ as.character(round(p.value, digits = 3)))) %>%
  pivot_wider(id_cols = c(term), 
              names_from = mod, #names_glue = ,
              values_from = c(estimate, p.value)) %>%
  select(term, estimate_bin_gain, p.value_bin_gain, everything())
