...
This document contains the primary R scripts for our traits analysis. 
The key code chunk is labeled "*many-models," which is where our three primary regression models are run (along with a few additional model specifications for exploration).
This chunk also then combines and saves the model results as a tibble, iterates through producing {gt} model coefficient tables, and saves model diagnostic plots.
All data files required for these models are included in the accompanying Zenodo data repository.

Script Contents:

0. Outline of Analyses, including the three primary models we report in our manuscript.
1. Model development, including exploring various species traits as predictor variables and developing and exploring various specifications of linear models.
2. Running the final linear model specifications, exploring model diagnostic plots and coefficient tables.
...
These two files contain data for the linear models involved in our traits analysis exploring how species' responses to cropland abandonment are affected by habitat suitabilities and other traits. This contains many variables, but key ones include the response variables for our models ("binary_gain_v_loss", "abs_change_percent_site", and "log(ratio)") and predictor variables c("forest_occ", "savanna_occ", "shrubland_occ", "grassland_occ", "wetlands_occ", "rocky_occ", "caves_occ", "desert_occ", "urban_occ", "arable_occ", "n_suitable_habitats_lvl2", "vert_class", "threatened", "Trophic_level", "log10(Body_mass_g)", "log10(total_range_area)", "abs(centroid_latitude)", and "max_abn_ext_percent_site"). Further details are contained in "traits.Rmd"

- final_species_list.csv
The final list of bird and mammal species included in our analysis, along with the overall response to cropland abandonment (overall_trend), the sites where that species had AOH affected by cropland abandonment (sites), the IUCN Red List Category, the obligate_type (i.e., whether a species is a mature forest obligate, or not), the the range size quantile (ranking species by global geographic range size), and common names. Note that mature forest obligates were excluded from our final results.

- site_df.csv
A list of site names and related information describing our study sites.

- trait_mod_df_modx1.rds
This R data file contains the results of our regression models run in the code chunk "many-models" in the "traits.Rmd" script. This consists of a tibble (i.e., a modified data frame in tidyverse parlance) containing the input data, resulting models, model coefficients, regression tables, and diagnostic statistics for our many model runs.
...
# ^^ see "AOH.Rmd" code chunk {r **feols-trends}
```

# 0. Outline of Analyses
...
1. Binary, proportion of winners (1) to non-winners (0), with one observation per species per site.
2. Change in AOH as a proportion of site area (Change in AOH / site area), with one observation for each species at each site.
3. Ratio of AOH at end of time series to AOH at start ($AOH_end / AOH_start$), log transformed.


```{r explore-tmp-files}
# ---------------------------------------------- #
# explore the following temporary input files: (created in "_util_files.R")
...
# see {r explore-tmp-files} and "_util_files.R" for files

# ---------- #
# 1. Binary winner or non-winner (i.e., proportion of winners):
# ---------- #
...
# ---------- #
# 2. change in AOH as a proportion of site area:
# ---------- #
# note: these are estimated changes in AOH based on OLS regression with NW estimator.
# response variable: abs_change_as_prop_site_area
...
# ---------- #
# 3. Ratio of AOH, log-transformed
# ---------- #
# Ratio of AOH at end of time series to AOH at start ($AOH_end / AOH_start$), log-transformed.

# response variable: log(ratio)
aoh_obs_change_tmp %>%
...
# 1. Etard, A., Morrill, S. & Newbold, T. Global gaps in trait data for terrestrial vertebrates. Global Ecology and Biogeography 29, 2143–2158 (2020).
# see: https://doi.org/10.1111/geb.13184
...
# note, we also considered using data from "EltonTraits" (Hamish Wilman, Jonathan Belmaker, Jennifer Simpson, Carolina de la Rosa, Marcelo M. Rivadeneira, and Walter Jetz. 2014. EltonTraits 1.0: Species-level foraging attributes of the world's birds and mammals. Ecology 95:2027. http://dx.doi.org/10.1890/13-1917.1) but decided against it.
...
# taxonomic common names, derived from HBW (birds) and Wikipedia (mammals), collected by hand.
...
```{r update-etard}
# update etard by matching bionomial names with IUCN
...
habitat_occurrence_df <- 
  habitat_age_req_coded %>% # see "IUCN.Rmd," Section 4
      mutate(
        ...
        degraded_forest = str_detect(suitable_habitats, regex("plantations|degraded", ignore_case = TRUE)),
        ...
        caves_occ =      str_detect(suitable_habitats, regex("caves", ignore_case = TRUE)),
        desert_occ =     str_detect(suitable_habitats, regex("desert", ignore_case = TRUE)),
        arable_occ =   str_detect(suitable_habitats, regex("arable", ignore_case = TRUE)),
        farmland_occ =   str_detect(suitable_habitats, regex("arable|pasture|garden", ignore_case = TRUE)),
        ag_occ =   str_detect(suitable_habitats, regex("arable|pasture|garden", ignore_case = TRUE)),
...
## Exploring Traits as Predictor Variables

How do the proportion of winner species compare across the following traits:
- trophic level (Trophic_level)
- body mass (Body_mass_g)
- range area
- range latitude ("tropicality")
- habitat breadth (number of suitable habitats)
- habitat preferences
...
gg_n_suitable_habitats_p_gain <-
  ...
  filter(aoh_type == "crop_abn_iucn") %>%
  group_by(aoh_type,
           n_suitable_habitats_lvl2,
           # n_suitable_habitats
           ) %>%
  summarise(p_gain = mean(overall_trend == "gain"), n_sp = n()) %>%
  ungroup() %>%
  ggplot(aes(
    # x = n_suitable_habitats,
    x = n_suitable_habitats_lvl2,
    # x = Habitat_breadth_IUCN,
    # x = as_factor(n_suitable_habitats),
    y = p_gain, label = n_sp, fill = desc(n_sp))) +
  theme_classic() +
  geom_col() + 
  geom_smooth(method = "lm", alpha = 0.2) + 
  geom_text(size = 2.25, nudge_y = +0.05) +
  geom_hline(yintercept = 0.5, linetype = "dashed") + 
    scale_x_continuous(n.breaks = 7) +
    labs(#title = "Impact of habitat specialization on response to abandonment", 
       y = "Proportion of species gaining habitat", x = "Number of suitable habitats (IUCN Level 2)",
       fill = "No. species") + 
...
gg_n_suitable_habitats_p_gain_by_vert_class <-
  aoh_mod_tmp_trends_by_sp %>%
  filter(aoh_type == "crop_abn_iucn") %>% 
  group_by(n_suitable_habitats_lvl2, aoh_type, vert_class) %>%
  summarise(p_gain = mean(overall_trend == "gain"), n_sp = n()) %>%
  ungroup() %>%
    # drop_na() %>%
  ggplot(aes(
    ...
    x = n_suitable_habitats_lvl2,
    # x = Habitat_breadth_IUCN,
    # x = as_factor(Habitat_breadth_IUCN),
    y = p_gain, label = n_sp, fill = desc(n_sp))) +
  theme_classic() +
  geom_col() + 
    geom_smooth(method = "lm", alpha = 0.2) + 
  ...
  scale_x_continuous(n.breaks = 7) +
    # scale_fill_viridis() +
  facet_grid(rows = vars(vert_class), #row = vars(aoh_type),
             # labeller = as_labeller(aoh_type_labels)
             )

# combo:

gg_n_suitable_habitats_p_gain_combo <-
  bind_rows(
  aoh_mod_tmp_trends_by_sp %>%
    filter(aoh_type == "crop_abn_iucn") %>%
    group_by(aoh_type, n_suitable_habitats_lvl2) %>%
    summarise(p_gain = mean(overall_trend == "gain"), n_sp = n()) %>% ungroup() %>%
    mutate(vert_class = "Birds + Mammals"),
  aoh_mod_tmp_trends_by_sp %>%
  filter(aoh_type == "crop_abn_iucn") %>% 
  group_by(aoh_type, n_suitable_habitats_lvl2, vert_class) %>%
  summarise(p_gain = mean(overall_trend == "gain"), n_sp = n()) %>%
  ungroup()) %>% mutate(vert_class = fct_relevel(vert_class, c("Birds", "Mammals", "Birds + Mammals"))) %>%
  ggplot(aes(x = n_suitable_habitats_lvl2, y = p_gain, label = n_sp, fill = desc(n_sp))) +
  theme_classic() +
  geom_col() + 
  geom_smooth(method = "lm", alpha = 0.2) + 
  geom_text(size = 2.25, nudge_y = +0.05) +
  labs(y = "Proportion of species gaining habitat", x = "Number of suitable habitats (IUCN Level 2)",
       fill = "No. species") + 
  geom_hline(yintercept = 0.5, linetype = "dashed") + 
  scale_x_continuous(n.breaks = 7) +
  facet_grid(rows = vars(vert_class))
    
ggsave(gg_n_suitable_habitats_p_gain_combo,
       filename = paste0(p_output, "plots/aoh/traits/", 
...
ggsave(gg_n_suitable_habitats_p_gain,
       filename = paste0(p_output, "plots/aoh/traits/", 
                           "n_suitable_habitats_lvl2_p_gain", ".pdf"),
...
ggsave(gg_n_suitable_habitats_p_gain_by_vert_class,
       filename = paste0(p_output, "plots/aoh/traits/", 
                           "n_suitable_habitats_lvl2_p_gain_by_vert", ".pdf"),
         width = 7, height = 4, units = "in"
       )
...
# gg_hab_occurrence_p_gain <-
  aoh_mod_tmp_trends_by_sp %>%
  filter(aoh_type == "crop_abn_iucn") %>% 
    select(aoh_type, vert_class, binomial, overall_trend, contains("occ")#, -n_hab_occ
           ) %>%
...
```{r plot-proportion-ag-users}
# --------------------------------------------------------------------------------- #
aoh_mod_tmp_trends_by_sp %>% names() %>% str_subset("occ")

# what proportion of species can use agriculture?
aoh_mod_tmp_trends_by_sp_all_incl_mature %>%
  filter(aoh_type == "crop_abn_iucn") %>%
  # habitat_occurrence_df %>%
  # left_join(aoh_species_list, by = "binomial") %>%
  # filter(binomial %in% unique(aoh_feols_trends_by_sp %>%
  ...
  group_by(aoh_type, 
           vert_class, 
           mature_forest_obligate = case_when(
             mature_forest_obl > 0.5 ~ "yes", 
             mature_forest_obl < 0.5 ~ "no"
             )) %>%
  summarise(n = n(), 
            p_arable = mean(arable_occ),
            p_forest = mean(forest_occ),
            p_savanna = mean(savanna_occ),
            p_shrubland = mean(shrubland_occ),
            p_grassland = mean(grassland_occ),
            p_wetlands = mean(wetlands_occ),
            p_urban = mean(urban_occ),
            p_rocky = mean(rocky_occ),
            p_desert = mean(desert_occ),
            p_caves = mean(caves_occ),
            # p_artificial = mean(artificial_occ)
            )
...
habitat_occurrence_df %>%
  ...
  filter(binomial %in% unique(aoh_feols_trends_by_sp %>% 
                                filter(aoh_type == "crop_abn_iucn", passage_type == "exclude_passage",
                                       vert_class != "amp") %>% pull(binomial))) %>%
  mutate(cropland_occ = str_detect(suitable_habitats, regex("arable", ignore_case = TRUE))) %>%
  select(aoh_type, vert_class, binomial, overall_trend, contains("occ"), agriculture_occ = farmland_occ, -n_hab_occ) %>%
  pivot_longer(cols = contains("occ"), names_to = "occurrence", values_to = "presence") %>%
  mutate(occurrence = gsub("_occ", "", occurrence)) %>%
  filter(presence == TRUE) %>%
  group_by(occurrence, aoh_type) %>%
...
# gg_prop_sp_w_habitat_suitability <-
  # aoh_mod_tmp_trends_by_sp_all_incl_mature %>%
  ...
  pivot_longer(cols = contains("occ"), names_to = "occurrence", values_to = "presence") %>%
  mutate(occurrence = gsub("_occ", "", occurrence)) %>%
  group_by(aoh_type, vert_class, occurrence, 
           mature_forest_obligate = #case_when(
             mature_forest_obl > 0.5# ~ "yes", mature_forest_obl < 0.5 ~ "no")
           ) %>%
  summarise(n = n(), p_occ = mean(presence), p_gain = mean(overall_trend == "gain")) %>%
  ungroup() %>%
    filter(!occurrence %in% c("farmland", "artificial_aquatic", 
                              "marine_coastal", "marine_intertidal",
                              "introduced"),
           mature_forest_obligate == "FALSE",
           occurrence == "desert") #%>%
  ggplot(aes(y = fct_reorder(occurrence, p_occ), x = p_occ)) +
  theme_classic() +
  geom_col() + 
  scale_x_continuous(limits = c(0,1), n.breaks = 6) +
  labs(x = "Proportion of species with suitability", y = "IUCN Habitat Type",
       # caption = "Note: 'farmland' includes species that occur in arable, pasture, or rural garden habitats"
       ) + 
  facet_grid(cols = vars(vert_class),
             # rows = vars(mature_forest_obligate),
             labeller = as_labeller(
               c("Birds" = "Birds", "Mammals" = "Mammals",
...
ggsave(gg_prop_sp_w_habitat_suitability,
       filename = paste0(p_output, "plots/aoh/traits/", 
                           "prop_sp_w_habitat_suitability", ".pdf"),
...
arable_gg_site_order_tmp <-
  aoh_est_change_tmp_all_incl_mature %>%
  filter(aoh_type == "crop_abn_iucn") %>%
  pivot_longer(cols = contains("occ"), names_to = "occurrence", values_to = "presence") %>%
  mutate(occurrence = gsub("_occ", "", occurrence)) %>%
  group_by(aoh_type, site, occurrence, 
           # vert_class,
           # trend,
           mature_forest_obligate = mature_forest_obl > 0.5
           ) %>%
  summarise(n = n(), p_occ = mean(presence), p_gain = mean(trend == "gain"),
            p_loss = mean(trend == "loss")) %>%
  ungroup() %>%
  filter(occurrence == "arable", mature_forest_obligate == "FALSE"#, vert_class == "Birds"
         ) %>%
  group_by(aoh_type, site) %>% mutate(richness = sum(n)) %>% ungroup() %>%
  mutate(n_winner = as.integer(p_gain*richness)) %>%
  select(site, richness, n_winner, p_occ, p_gain, p_loss) %>%
  arrange(desc(p_gain))

arable_gg_site_order_tmp %>% 
  select(-p_occ) %>%
  # arrange(desc(n_winner)) %>%
  flextable() %>%
  colformat_double(., digits = 2)

arable_gg_site_order_tmp %>%
  # filter(trend == "gain") %>%
  mutate(n_winner = p_gain*richness)

  

aoh_est_change_tmp_all %>%
  filter(aoh_type %in% c("crop_abn_iucn", "full_iucn")) %>%
  select(aoh_type, vert_class, site, binomial, trend, contains("occ")) %>%
  pivot_longer(cols = contains("occ"), names_to = "occurrence", values_to = "presence") %>%
  mutate(occurrence = gsub("_occ", "", occurrence)) %>%
  # filter(occurrence == "arable", site == "belarus") #%>%
  group_by(aoh_type, site, occurrence,    # vert_class
           ) %>%
  summarise(n = n(), 
            p_occ = mean(presence), 
            p_gain = mean(trend == "gain"),
            p_loss = mean(trend == "loss"),
            ) %>%
  ungroup() %>%
  # filter(site == "belarus")
  filter(occurrence == "arable", # vert_class == "Birds"
         ) %>%
    select(aoh_type, site, p_occ, p_gain, p_loss) %>%
  arrange(
    p_loss
    # p_gain
    ) %>%
...
# proportion of winners and species with arable suitability
ggsave(
  aoh_est_change_tmp_all_incl_mature %>%
    # aoh_est_change_tmp %>%
    filter(aoh_type %in% c("crop_abn_iucn"#, "full_iucn"
    ...
    pivot_longer(cols = contains("occ"), names_to = "occurrence", values_to = "presence") %>%
    mutate(occurrence = gsub("_occ", "", occurrence)) %>%
    group_by(aoh_type, vert_class, site, occurrence, 
             mature_forest_obligate = mature_forest_obl > 0.5
             ) %>%
    summarise(n = n(), p_occ = mean(presence), p_gain = mean(trend == "gain")) %>%
    ungroup() %>%
    filter(occurrence == "arable", 
           mature_forest_obligate == "FALSE"
           ) %>%
    pivot_longer(cols = c(p_occ, p_gain), names_prefix = "p_", names_to = "p_type", values_to =
                   "proportion") %>%
    left_join(arable_gg_site_order_tmp) %>%
    ggplot(aes(y = fct_reorder(site, #p_gain
                               p_occ), x = proportion, group = p_type, fill = p_type)) +
    theme_classic() +
    geom_col(position = position_dodge()) +
    scale_y_discrete(labels = fancy_labels) +
    scale_x_continuous(limits = c(0,1), n.breaks = 6) +
    labs(x = "Proportion", y = NULL, fill = NULL) +
    scale_fill_manual(labels = c("gain" = "Proportion with gains",
                                 "occ" = "Proportion arable suitable"), 
                      values = c("#3b7c70", "#ce9642")) +
    facet_grid(cols = vars(vert_class),
               # rows = vars(aoh_type),
               # rows = vars(mature_forest_obligate),
               labeller = as_labeller(c("Birds" = "Birds", "Mammals" = "Mammals",
                                        "TRUE" = "Mature Forest Obligates", 
                                        "FALSE" = "Non-Mature Forest Obligates"))) + 
    theme(legend.position = "bottom"),
  
  filename = paste0(p_output, "plots/aoh/traits/", 
                    "prop_sp_w_arable_suitability", ".pdf"),
  width = 5, height = 4.5, units = "in"
  )



# proportion of winners as a function of the proportion with arable suitability
aoh_est_change_tmp_all_incl_mature %>%
  filter(aoh_type == "crop_abn_iucn") %>%
  pivot_longer(cols = contains("occ"), names_to = "occurrence", values_to = "presence") %>%
  mutate(occurrence = gsub("_occ", "", occurrence)) %>%
  ...
  summarise(n = n(), p_occ = mean(presence), p_gain = mean(trend == "gain")) %>%
  ungroup() %>%
  filter(occurrence == "arable", mature_forest_obligate == "FALSE") %>%
  # pivot_longer(cols = c(p_occ, p_gain), names_prefix = "p_", names_to = "p_type", values_to = "proportion") %>%
  left_join(arable_gg_site_order_tmp) %>%
  ggplot(aes(x = p_occ, y = p_gain, col = vert_class)) +
  theme_classic() +
  geom_point() +
  geom_smooth(method = "lm") +
  facet_grid(rows = vars(mature_forest_obligate),
             labeller = as_labeller(c("Birds" = "Birds", "Mammals" = "Mammals",
                 "TRUE" = "Mature Forest Obligates", 
                 "FALSE" = "Non-Mature Forest Obligates")))
...
# -------------------------------------------------------- #
# Latitude v. Arable suitability
ggsave(
  aoh_est_change_tmp_all_incl_mature %>%
  select(binomial, arable_occ, artificial_terrestrial_occ,
         centroid_latitude, mature_forest_obl#, trend, passage_type, aoh_type
         ) %>%
  unique() %>%
  ggplot(mapping = aes(#x = artificial_terrestrial_occ,
                       x = arable_occ,
                       y = abs(centroid_latitude))) + 
    # geom_violin() + 
  geom_boxplot(notch = TRUE, width = 0.9) +
  geom_point(position = position_jitter(width = 0.3), alpha = 0.15) + 
  theme_classic() + coord_flip() +
  labs(y = "Distance of range centroid from \nEquator (degrees latitude)", x = "Able to occupy arable land"),
...
aoh_est_change_tmp_all_incl_mature %>%
  ...
  geom_histogram() +
  theme_classic() +
  labs(x = "Distance of range centroid from \nEquator (degrees latitude)") +
  facet_grid(vars(arable_occ))

```

```{r range-centroid}

# load centroids calculated in qgis:
...
# translate centroids into coordinates:
centroids_df <- 
  bind_rows(
    bird_centroids %>%
...
# temp #
trait_mod_df[2,]$model[[1]]
trait_mod_df[2,]$model[[1]]$deviance
trait_mod_df[2,]$model[[1]]$null.deviance
summary(trait_mod_df[2,]$model[[1]])
tidy(trait_mod_df[2,]$model[[1]])
glance(trait_mod_df[2,]$model[[1]])

100 * (1-839.49/1495.68)

100*with(summary(trait_mod_df[2,]$model[[1]]), 1 - deviance/null.deviance)
...
binary_mod_gain_v_loss <-
  glm(binary_gain_v_loss ~ 
        forest_occ + savanna_occ + shrubland_occ + grassland_occ + wetlands_occ + artificial_occ + n_hab_occ +
        threatened + Trophic_level + 
        log(Body_mass_g) + log(total_range_area) + abs(centroid_latitude) + 
        max_abn_extent_div_site_area,
      data = aoh_est_change_tmp,
...
glm(binary_gain_v_loss ~ 
      forest_occ + savanna_occ + shrubland_occ + 
      grassland_occ + wetlands_occ +
      rocky_occ + caves_occ + desert_occ +
      # marine_intertidal_occ + marine_coastal_occ +
      # artificial_terrestrial_occ +
      # urban_occ +
      # farmland_occ +
      # arable_occ +
      # ag_occ + 
      ...
      threatened + Trophic_level +
      log(Body_mass_g) + log(total_range_area) + abs(centroid_latitude) +
      max_abn_ext_percent_site,
    data = aoh_obs_change_tmp_all %>% filter(aoh_type == "crop_abn_iucn"),
    family = "binomial") %>%
  summary()
...
lm(abs_change_percent_site ~ 
       forest_occ + savanna_occ + shrubland_occ + grassland_occ + wetlands_occ + 
            rocky_occ + caves_occ + desert_occ + artificial_terrestrial_occ + 
            n_suitable_habitats +
     threatened + Trophic_level + 
       log(Body_mass_g) + log(total_range_area) + abs(centroid_latitude) + 
       max_abn_ext_percent_site,
...
lm(abs_change_percent_site ~ 
     ...
     vert_class + 
     threatened + Trophic_level +
     log10(Body_mass_g) + log10(total_range_area) + abs(centroid_latitude) +
       max_abn_ext_percent_site,
    data = aoh_obs_change_tmp_all %>% filter(aoh_type == "crop_abn_iucn")
     ) %>%
  summary()
...
predictor_variables_all <- c(
  "forest_occ", "savanna_occ", "shrubland_occ", "grassland_occ", "wetlands_occ",
  "rocky_occ", "caves_occ", "desert_occ",
  "urban_occ", "arable_occ", 
  # "marine_intertidal_occ", "marine_coastal_occ"
  "artificial_terrestrial_occ",
  "n_suitable_habitats_lvl2", 
  "vert_class", "threatened", "Trophic_level", 
  "log10(Body_mass_g)", "log10(total_range_area)", "abs(centroid_latitude)",
  "max_abn_ext_percent_site")

# iterate through various specifications:
predictor_variables <- predictor_variables_all[-c(11)]
mod_label <- "_mod1"

predictor_variables <- predictor_variables_all[-c(9:10)]
mod_label <- "_mod2"

predictor_variables <- predictor_variables_all[-c(9:11)]
mod_label <- "_mod3"


# ------------------------------------- #
# Run glm and lm models --------------- #
# ------------------------------------- #

# This lapply call iterates through three subsets of predictor variables.
# The first, predictor_variables_all[-c(11)], is the final subset.
# This lapply call runs the glm models, with glm_response_variables, and stores the models in a tibble.
# It then runs the lm models, and then combines those models into a single data frame to be saved.
# It also saves a bunch of gt tables with the model outcomes, and a few diagnostic plots.

# --------------- start lapply() call -------------------------- #

lapply(1, # 1:3, # only for exploration, determining the final predictor variable mix
       function(i) 
  {
  predictor_variables_l <- list(
    predictor_variables_all[-c(11)], # final model specification
    predictor_variables_all[-c(9:10)],
    predictor_variables_all[-c(9:11)]
    )
  predictor_variables <- predictor_variables_l[[i]]
  mod_label <- paste0("_modx", 1:3)[i]
...
glm_mod_df <- 
  ...
  mutate(model = map2(data, response, 
                      ~glm(as.formula(paste0(
                        as.character(.y), paste0(" ~ ", paste(predictor_variables, collapse = " + ")))), 
...
# Save
# ------------------------------------- #
saveRDS(trait_mod_df, file = paste0(p_derived, "traits/", "trait_mod_df", mod_label,".rds"))
...
trait_mod_df %>% 
  ...
  gtsave(filename = paste0(p_plots, "aoh/traits/", "Table1b_binary_obs", mod_label,".png"))
...
trait_mod_df %>% 
  ...
  gtsave(filename = paste0(p_plots, "aoh/traits/", "Table2_3_obs_change_AOH_combined", mod_label,".png"))
...
trait_mod_df %>% 
  ...
  gtsave(filename = paste0(p_plots, "aoh/traits/", 
                           "Table4_obs_ratios", mod_label,".png"))
...
trait_mod_df %>% 
  ...
  text_transform(
    locations = cells_body(),
    fn = function(x) {str_replace_all(x, pattern = "km2", replacement = "km<sup>2</sup>")}) %>%
  gtsave(filename = paste0(p_plots, "aoh/traits/", "Table2_3_7_obs_AOH_change", mod_label,".png"))
...
trait_mod_df %>% 
  ...
  text_transform(
    ...
    fn = function(x) {str_replace_all(x, pattern = "km2", replacement = "km<sup>2</sup>")}) %>%
  gtsave(filename = paste0(p_plots, "aoh/traits/",
                           "Table_final_models_v2", mod_label,".png"))
...
trait_mod_df %>% 
  ...
  text_transform(
    ...
    fn = function(x) {str_replace_all(x, pattern = "km2", replacement = "km<sup>2</sup>")}) %>%
  gtsave(filename = paste0(p_plots, "aoh/traits/",
                           "Table_final_models_v3", mod_label,".png"))
...
pdf(file = paste0(p_plots, "aoh/traits/",
                  "model_diagnostics_resid_v_fitted_combo", mod_label,".pdf"),
    width = 6.5, height = 9)
par(mfrow = c(4,3))
...
pdf(file = paste0(p_plots, "aoh/traits/",
                  "model_diagnostics_qq_combo", mod_label,".pdf"),
    width = 6.5, height = 9)
par(mfrow = c(4,3))
lapply(1:10, function(i) {
  plot(trait_mod_df %>% 
...
# ---------------------------------------------------- #
# ---- Compare Observed and Potential Abandonment ---- #

# Simplify tables:
trait_mod_df %>%
  filter(response %in% c("binary_gain_v_loss", "abs_change_percent_site", "log(ratio)")) %>%
...
glm_mod_by_sp_df <-
  ...
  mutate(model = map2(data, response, 
                      ~glm(as.formula(paste0(
                        as.character(.y), 
                        paste0(" ~ ", paste(predictor_variables[-length(predictor_variables)], collapse = " + ")))), 
                      data = .x, family = "binomial")),
    tidy = lapply(model, tidy),
...
## Explore Diagnostic Plots & Coefficient Tables

Note: model coefficient tables presented in manuscript are produced directly in "biodiv_abn_MS.Rmd"
...
# ------------------------------------------------ #
# merge and save model tables into combination tables
...
```{r explore-mod-differences}

trait_mod1_df <- readRDS(file = paste0(p_derived, "traits/", "trait_mod_df_lite_mod1.rds")) %>%
  select(-c(gt)) %>%
  mutate(mod_run = "mod1")

trait_mod2_df <- readRDS(file = paste0(p_derived, "traits/", "trait_mod_df_lite_mod2.rds")) %>%
  select(-c(gt)) %>%
  mutate(mod_run = "mod2")

trait_mod3_df <- readRDS(file = paste0(p_derived, "traits/", "trait_mod_df_lite_mod3.rds")) %>%
  select(-c(gt)) %>%
  mutate(mod_run = "mod3")

tmod_df <- 
  bind_rows(readRDS(file = paste0(p_derived, "traits/", "trait_mod_df_lite_mod1.rds")) %>% 
              mutate(mod_run = "arable_urban"),
            readRDS(file = paste0(p_derived, "traits/", "trait_mod_df_lite_mod2.rds")) %>% 
              mutate(mod_run = "art"),
            readRDS(file = paste0(p_derived, "traits/", "trait_mod_df_lite_mod3.rds")) %>% 
              mutate(mod_run = "none")
            ) %>%
  select(-c(data, model, glance)) %>%
  unnest(c(tidy))
...
tmod_df %>%
  filter(aoh_type == "crop_abn_iucn", 
         mod_run != "art",
         mod_run == "arable_urban"
         # mod_run == "none"
         ) %>%
  mutate(
    estimate = case_when(p.value < 0.05 ~ estimate),
    # exponentiate log(ratio) coefs
    estimate = case_when(response == "log(ratio)" ~ exp(1)^estimate, 
                         str_detect(response, "binary") ~ exp(1)^estimate, 
                         TRUE ~ estimate),
    estimate = round(estimate, digits = 2)
    ) %>%
  
  # recreating final table v3
  filter(response %in% c("binary_gain_v_loss", "abs_change_percent_site", "log(ratio)")) %>%
  ...
  # 
  # binary variables:
  # filter(response %in% c("binary_trend_gain", "binary_gain_v_loss", "binary_trend_loss", "binary_trend_no_trend")) %>%
  # mutate(response = case_when(response == "binary_trend_gain" ~ "gain",
  #                             response == "binary_gain_v_loss" ~ "gl",
  #                             response == "binary_trend_loss" ~ "loss",
  #                             response == "binary_trend_no_trend" ~ "no_trend")) %>%

  # aoh change variables
  # filter(response %in% c(lm_response_variables[c(1,2,4)], lm_response_variables_slope[2])) %>%
  # mutate(response = case_when(response == "abs_change" ~ "AOH_change",
  #                             response == "abs_change_percent_site" ~ "AOH_percent",
  #                             response == "log(ratio)" ~ "log_ratio",
  #                             response == "slope_prop_site*10^4" ~ "slope_percent")) %>%

# all variables:
  # filter(response %in% c("binary_trend_gain", "binary_gain_v_loss", "binary_trend_loss", "binary_trend_no_trend",
  #                        lm_response_variables[c(1,2,4)], lm_response_variables_slope[2])) %>%
  # mutate(response = case_when(response == "binary_trend_gain" ~ "gain",
  #                             response == "binary_gain_v_loss" ~ "gl",
  #                             response == "binary_trend_loss" ~ "loss",
  #                             response == "binary_trend_no_trend" ~ "no_trend",
  #                             response == "abs_change" ~ "AOH_change",
  #                             response == "abs_change_percent_site" ~ "AOH_percent",
  #                             response == "log(ratio)" ~ "log_ratio",
  #                             response == "slope_prop_site*10^4" ~ "slope_percent")) %>%
  
  select(mod_run, response, term, estimate) %>%
  pivot_wider(id_cols = c(mod_run, term), names_from = "response", values_from = "estimate") %>%
  # pivot_wider(id_cols = c(response, term), names_from = "mod_run", values_from = "estimate") %>%
  # arrange(response) %>%
  mutate(term = fct_relevel(term, c("(Intercept)","forest_occTRUE", "savanna_occTRUE", "shrubland_occTRUE", "grassland_occTRUE", "wetlands_occTRUE", "rocky_occTRUE", "caves_occTRUE", "desert_occTRUE", "urban_occTRUE", "arable_occTRUE", "artificial_terrestrial_occTRUE", "n_suitable_habitats_lvl2", "vert_classMammals", "threatenedThreatened", "Trophic_levelHerbivore", "Trophic_levelOmnivore", "log(Body_mass_g)", "log(total_range_area)", "abs(centroid_latitude)", "max_abn_ext_percent_site"))) %>%
  arrange(term) %>%
  flextable() %>%  theme_booktabs() %>%
  save_as_image(path = paste0(p_plots, "aoh/traits/",
                              # "Comparing_trait_models_binary.png"
                              # "Comparing_trait_models_aoh_magnitude.png"
                              # "Comparing_trait_models_final.png"
                              # "Comparing_trait_models_all.png"
                              # "Trait_mod1_table_all.png"
                              # "Trait_mod3_table_all.png"
                              "Trait_mod1_table_trim.png"
                              # "Trait_mod3_table_trim.png"
                              ))
