...
# Birds
if(vert == "bird") {
  range_sf <- st_read(dsn = paste0(p_range,"bird_clip_qgis.shp"))
}
...
#
# df <- range_sf %>% st_drop_geometry() %>% as_tibble()
# df1 <- bird_valid %>% st_drop_geometry() %>% as_tibble()
# df
# df[, c(1, 12:18)]
# names(df)
# df$presence %>% unique()
...
save(range_sf, file = paste0(p_range, vert, "_sf_validated.RData"))
