# ------------------------------------------------------------ #
# Convert abn_lc to data.table, pass temporal filter
# 
...
# convert abn_lc SpatRaster to data.table
dt <- spatraster_to_dt(abn_lc[[site_index]])

# write to file
fwrite(dt, 
       file = paste0(
         p_derived,
         site_df$site[site_index], "_abn_lc.csv")
       )
...
fwrite(dt, 
       file = paste0(
         p_derived, 
         site_df$site[site_index], "_abn_lc_clean.csv"))


cat(paste0("Success! Converted abn_lc to data.table & passed temporal filter, for site: ", site_df$site[site_index]))
