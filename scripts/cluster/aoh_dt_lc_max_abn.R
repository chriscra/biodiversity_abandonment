...
# for Yin et al. 2020 land cover, exclusively within pixels that were 
# abandoned at some point during the time series (masking with max_age dt)
# 
...
hab_dt <- fread(input = paste0(p_input_rasters, site_df$site[site_index], "_clean.csv")#,
                #select = 1:3
                )
max_age <- fread(input = paste0(p_input_rasters, site_df$site[site_index], "_max_age_2021_03_13.csv"))
...
# merge hab_dt with max_age

hab_dt <- merge(hab_dt, max_age, all = TRUE, by = c("x", "y"), sort = FALSE)


# run AOH function
aoh_max_abn_lc_tmp <- lapply(1:nrow(species_list_tmp), 
                  function(i) {
  cc_AOH_dt_max_age(index = i, 
...
# write to file
write_csv(aoh_max_abn_lc_tmp,
          file = paste0(p_derived, "aoh_max_abn_lc_tmp", time_stamp, "_c", core, ".csv")
          )

cat(paste0("Wrote aoh_tmp file for core ", core, " (",site_df$site[site_index], ") to:", 
           p_derived, "aoh_max_abn_lc_tmp", time_stamp, "_c", core, ".csv"), fill = TRUE)
