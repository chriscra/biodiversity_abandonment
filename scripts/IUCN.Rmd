...
```{r dolphins}
common_names %>% filter(binomial == "Inia geoffrensis")
habitat_details %>% filter(binomial == "Inia geoffrensis")

habitat_age_req %>% select(binomial, common_names, habitat)

species_ranges %>%
  st_drop_geometry() %>%
  filter(freshwater == "true", vert_class %in% c("bird", "mam")) %>%
  as_tibble() %>%
  select(site, vert_class, binomial, marine:freshwater) %>%
  left_join(habitat_age_req %>% select(binomial, common_names, habitat)) %>%
  print(n = 50)
```
...
habitat_age_req_2022_12_06 %>% filter(is.na(coder))

habitat_age_req_2022_12_06 %>%  
  filter(str_detect(mature_forest_obl, "NA") | is.na(mature_forest_obl))

habitat_age_req_2022_12_06 %>%  
  filter(str_detect(water_obl, "NA") | is.na(water_obl))
...
habitat_age_req_2022_04_05_CC$vert_class %>% unique()
habitat_age_req$vert_class %>% unique()
habitat_age_req %>% filter(vert_class != "amp")
habitat_age_req_coded %>% filter(vert_class != "amp")
