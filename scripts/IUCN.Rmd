...
# comparing_times0 <- aoh_31_df %>%
# comparing_times_abn <- aoh_abn %>%
comparing_times_lc <- aoh_lc_df %>%
...
comparing_times0
comparing_times0 %>% select(site, max_time, mean_time, ncell, record_multiplier_mn)
comparing_times_abn %>% select(site, max_time, mean_time, ncell, record_multiplier_mn)
comparing_times_lc %>% select(site, max_time, mean_time, ncell, record_multiplier_mn)
...
core_tests_abn %>% arrange(diff) %>% print(n = 20)
...
calc_cores <-
  lapply(c("abn", "lc"), function(i) {
    comparing_times %>% 
      ...
      left_join(records_by_site, by = "site") %>%
      mutate(type = i,
             sp_per_core = filter(block_sizes, type == i)$block * record_multiplier,
             cores_needed = runs/sp_per_core,
             cores_rounded = ceiling(cores_needed),
             runs_per_core = ceiling(runs/cores_rounded),
             hr_if_max = runs_per_core * max_time / 60 / 60,
