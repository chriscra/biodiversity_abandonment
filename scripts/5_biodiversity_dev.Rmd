...
p_range <- "/Volumes/GoogleDrive/My Drive/_Projects/Zambia/agroEcoTradeoff/external/data_new/1_IUCN_dev/"
...
```{r pnv}
# load PNV
pnv_1km <- raster(paste0(p_dat, "/Habitats/PNV/Hengl_2018_PNV/1km/pnv_biome.type_biome00k_c_1km_s0..0cm_2000..2017_v0.1.tif"))
...
pnv <- rast(paste0(p_dat, "/Habitats/PNV/Hengl_2018_PNV/250m/pnv_biome.type_biome00k_c_250m_s0..0cm_2000..2017_v0.2.tif"))
plot(pnv)
...
# color palette information
pnv_table <- read_csv(paste0(p_dat, "Habitats/PNV/Hengl_2018_PNV/250m/pnv_biome.type_biome00k_c_250m_s0..0cm_2000..2017_v0.2.tif.csv"))

pnv_table <- pnv_table %>% 
  mutate(labels = paste0(Number, ": ", New.global.consolidated.biome.scheme),
...
site_pnv_freq <- lapply(1:11, function(i) {
  freq(site_pnv[[i]]) %>% as_tibble() %>% mutate(site = site_df$site[i])
  }) %>% bind_rows()
...
site_pnv <- lapply(1:11, function(i) {
  terra::crop(pnv, ext(lc[[i]]),
              filename = paste0(p_derived, "site_pnv/", names(lc[i]), "_pnv.tif"),
              overwrite = TRUE
              )
  }
  )
...
# load back in:
site_pnv <- lapply(
  list.files(paste0(p_derived, "site_pnv"), full.names = TRUE) %>% grep("_pnv.tif", ., value = TRUE), 
  function(i) rast(i)
  )
...
rs2 <- terra::resample(
    x = site_pnv[[i]], y = age_t[[i]]$y2017, method = "near")
...
for(i in 1:11) {plot(site_pnv[[i]], 
                     main = names(site_pnv[i]),
                     mar = c(mval+0.5, mval, mval + 1.2, mval + 2), # bltr     
                     type = "classes", #legend = FALSE,
                     col = pnv_table %>% 
                       filter(Number %in% 
                                filter(site_pnv_freq, site == site_df$site[i])$value) %>% 
                       .$cols
)
  }
...
# crop pnv to conus
pnv_250 <- rast(paste0(p_dat, "/Habitats/PNV/Hengl_2018_PNV/250m/pnv_biome.type_biome00k_c_250m_s0..0cm_2000..2017_v0.2.tif"))
...
# level 1
habitat <- raster(paste0(p_dat, "Habitats/Jung_GlobalHabitatTypes/iucn_habitatclassification_composite_lvl1_ver003.tif"))

# level 2
habitat <- raster(paste0(p_dat, "Habitats/Jung_GlobalHabitatTypes/iucn_habitatclassification_composite_lvl2_ver003.tif"))
...
```{r load-cp}
forest_c <- rast(paste0(p_dat, "climate/carbon_seq_rates/sequestration_rate_layer_in_full_extent.tif"))
plot(forest_c)
...
site_forest_c <- lapply(1:11, function(i) {
  terra::crop(forest_c, ext(lc[[i]]),
              filename = paste0(p_derived, "site_forest_carbon/", names(lc[i]), "_forest_c.tif"),
              overwrite = TRUE
              )
  }
  )
