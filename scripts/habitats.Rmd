...
potential_age_t[[site_index]]$y2017 %>% plot(ext = small_cq)

tt <- crop(abn_lcc[[site_index]], small_cq)
ait <- crop(abn_lcc_iucn_habitat[[site_index]], small_cq)
ptt <- crop(potential_age_t[[site_index]], small_cq)
mtt <- crop(max_age_t[[site_index]], small_cq)
mt2 <- crop(max_age_mask[[site_index]], small_cq)
...
Regardless, I need to produce a version of the potential_abn_lcc_iucn layer that includes the habitat before and after, to match the max_abn_lcc_iucn layer.
...
Perhaps I could update all grassland codes in one year with the values from the previous year. 
...
plot(potential_abn_lcc_iucn_habitat$chongqing$y2017, ext = small_cq)
plot(full_test, ext = small_cq)
...
# compare chongqing
site_index <- 3
plot(potential_abn_lcc_iucn_habitat[[site_index]]$y2017, ext = small_cq)
...
freq(max_abn_lcc[[9]][[1:2]]) %>% as_tibble() %>% 
    mutate(site = site_df$site[i],
           year = layer + 1986) %>%
  rename(lc = value)
