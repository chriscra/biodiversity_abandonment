...
## Direct assignment of lc to iucn habitat
```{r direct-assignment}
site_index <- 9
...
resam1 <- terra::resample(rcl_iucn, rcl_lc, method = "near")
resam2 <- terra::resample(rcl_lc, rcl_iucn, method = "near")
...
r <- rast(nrow=10, ncol=10, crs='+proj=utm +zone=1 +datum=WGS84', 
...
x <- terra::focal(r, w = 3, fun = function(i) {modal(i, ties = "first", na.rm = TRUE)},
...
x1 <- terra::focal(r, w = 3, 
                   # fun = function(i) {modal(i, na.rm = TRUE)},
                  fun = "modal",
                  na.rm = TRUE,
                  na.only = TRUE, expand = TRUE)
plot(r)
