...
4. Miscellaneous spatial data, including site area, elevation, and ecoregions & biomes for each site.
...
# this chunk creates "jung_hab_type_area_df" - used when calculating AOH for land cover classes [class_type == "lc"], instead of iucn habitat types directly.
...
# 3. Assign Yin et al. 2020 Land Covers to IUCN Level 2 Habitat Types
...
## Yin et al. 2020 land cover classes in abandoned croplands 

```{r create-abn-age-masks}
# These masks are used in later code, 
...
abn_lcc <- lapply(1:11, function(i) {
  # make sure to restrict lcc to only 1987:2017 (Nebraska, Wisconsin)
  # i <- 9
...
# ---------------------------------------------------------------- #
# calculate land cover (Yin et al. 2020) of all pixels that were ever abandoned
# ---------------------------------------------------------------- #
...
# write to file
write_csv(abn_lcc_freq, file = paste0(p_derived, "abn_lcc_freq.csv"))
...
```{r frag-df}
# load fragmentation data (from landscape metrics functions) derived from computing clusters
...
write_csv(frag_df, paste0(p_derived, "frag/frag_df.csv"))


# hypothetical, assuming no abandonment through the year 2017
frag_hypo_df <- 
  lapply(site_df$site, function(i) {
...
write_csv(frag_hypo_df, paste0(p_derived, "frag/frag_hypo_no_abn_2017_df.csv"))
