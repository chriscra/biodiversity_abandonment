...
gg_crosswalk_jac <- 
  ggplot(data = crosswalk_jac, 
         mapping = aes(x = as_factor(lc), y = jaccard, fill = as_factor(ver))) +
...
iucn_tmp <- rep(site_jung_l2[[site_index]], length(lc_classes))# %>% crop(., lcc[[site_index]])
names(iucn_tmp) <- c("lc2", "lc4")

iucn_sub <- lapply(seq_along(lc_classes), function(i) {
  lc_class <- lc_classes[i]
  
  classify(
    iucn_tmp[[i]], # crop first, since focal takes care of NA border cells
    rcl = iucn_crosswalk %>% 
...
# just a single land cover class:
...
writeRaster(iucn_focal,
            filename = paste0(p_derived, "iucn_habitat_lcc_composite/", 
                              site_df$site[site_index], "_iucn_focal.tif"),
            names = "iucn_focal_composite",
            overwrite = TRUE)
...
iucn_focal_resampled <- 
  ...
  terra::resample(
    x = iucn_focal[[i]], y = lcc_sub[[i]], method = "near",
    # filename = paste0(p_derived, "site_jung/", names(lc[i]), "_jung_l2_30.tif"),
    # names = paste0(names(lc[i]), "_jung_l2_30"),
    # overwrite = TRUE
    )
...
iucn_focal_msk <- lapply(seq_along(lc_classes), function(i) {
  terra::mask(
  # these need to have the same number of layers:
    iucn_focal_resampled[[i]], lcc[[site_index]],
    maskvalues = lc_classes[i], inverse = TRUE,
    names = names(lcc[[site_index]]),
    filename = paste0(p_derived, "iucn_habitat_lcc_composite/", site_df$site[site_index],
...
lcc[[site_index]][[28:31]]


# [unused] Alternatively: reclassify land cover map to extract just one land cover class (e.g., 2, woody vegetation).
# This will be used to mask the resampled IUCN habitat map.
# 
# lcc_tmp <- rep(lcc[[site_index]][[29]], length(lc_classes))
# names(lcc_tmp) <- paste0("lc", lc_classes) #c("lc2", "lc4")
# lcc_tmp
# 
# tic()
# lcc_sub <- lapply(seq_along(lc_classes), function(i) {
#   lc_class <- lc_classes[i]
#   
#   lc_sub_tmp <- classify(
#     # lcc[[site_index]][[25:29]],
#     lcc_tmp[[i]],
#     # simple rcl, e.g., where 2 stays 2, everything else is converted to NA
...
#   }) %>% rast()
# toc()
# names(lcc_sub) <- paste0("lc", lc_classes)
# 
# plot(lcc_sub)
# names(lcc_sub)
# 
...
# writeRaster(
#   lcc_sub,
#   filename = paste0(p_tmp, site_df$site[site_index], "_lcc_sub.tif"),
#         overwrite=TRUE)
...
# 5. Construct a composite habitat map for each year by knitting together these four resampled habitat maps
# (Either by converting NAs to zeros and adding, or some other method.)
iucn_focal_msk %>% length()
tic()
iucn_focal_composite <- cover(iucn_focal_msk[[1]], iucn_focal_msk[[2]])
if (length(iucn_focal_msk) > 2) {
  iucn_focal_composite <- cover(iucn_focal_composite, iucn_focal_msk[[3]])
}

if (length(iucn_focal_msk) > 3) {
  iucn_focal_composite <- cover(iucn_focal_composite, iucn_focal_msk[[4]])
}
toc()

plot(iucn_focal_composite[[28:31]])


# save this composite layer:
writeRaster(iucn_focal_composite,
            filename = paste0(p_derived, "iucn_habitat_lcc_composite/", 
                              site_df$site[site_index], "_iucn_focal_composite.tif"),
            names = names(iucn_focal_composite),
            overwrite = TRUE)
...
### Resample based on modal value using `terra::focal()`



### Assigning via nearest neighbor
...
pdf(file = paste0(p_output, "plots/land_cover_1987.pdf"), width = 10, height = 7.5)
mval <- 1.5
par(mfrow = c(3, 4), 
      # mar=c(0,0,0,0), 
      oma=c(1,1,0,0),
...
for(i in 1:11) {
  plot(lcc[[i]]$y1987, col = lc_plot_cols$color, levels = lc_plot_cols$name, 
